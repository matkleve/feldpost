-- =============================================================================
-- Share sets for workspace export links
-- =============================================================================

create table if not exists public.share_sets (
  id uuid primary key default gen_random_uuid(),
  organization_id uuid not null references public.organizations(id) on delete cascade,
  created_by uuid references auth.users(id) on delete set null,
  token_hash text not null unique,
  token_prefix text not null,
  fingerprint text not null,
  expires_at timestamptz,
  revoked_at timestamptz,
  created_at timestamptz not null default now()
);

create table if not exists public.share_set_items (
  share_set_id uuid not null references public.share_sets(id) on delete cascade,
  image_id uuid not null references public.images(id) on delete cascade,
  item_order int not null,
  created_at timestamptz not null default now(),
  primary key (share_set_id, image_id)
);

create unique index if not exists idx_share_sets_org_fingerprint_active
  on public.share_sets (organization_id, fingerprint)
  where revoked_at is null;

create index if not exists idx_share_sets_lookup_active
  on public.share_sets (organization_id, token_hash)
  where revoked_at is null;

create index if not exists idx_share_set_items_share_order
  on public.share_set_items (share_set_id, item_order);

alter table public.share_sets enable row level security;
alter table public.share_set_items enable row level security;

drop policy if exists "share_sets: org read active" on public.share_sets;
create policy "share_sets: org read active"
  on public.share_sets for select
  using (
    organization_id = public.user_org_id()
    and revoked_at is null
    and (expires_at is null or expires_at > now())
  );

drop policy if exists "share_sets: own or admin insert" on public.share_sets;
create policy "share_sets: own or admin insert"
  on public.share_sets for insert
  with check (
    organization_id = public.user_org_id()
    and created_by = auth.uid()
    and not public.is_viewer()
  );

drop policy if exists "share_sets: owner or admin update" on public.share_sets;
create policy "share_sets: owner or admin update"
  on public.share_sets for update
  using (
    organization_id = public.user_org_id()
    and (created_by = auth.uid() or public.is_admin())
    and not public.is_viewer()
  )
  with check (organization_id = public.user_org_id());

drop policy if exists "share_sets: owner or admin delete" on public.share_sets;
create policy "share_sets: owner or admin delete"
  on public.share_sets for delete
  using (
    organization_id = public.user_org_id()
    and (created_by = auth.uid() or public.is_admin())
    and not public.is_viewer()
  );

drop policy if exists "share_set_items: parent read" on public.share_set_items;
create policy "share_set_items: parent read"
  on public.share_set_items for select
  using (
    exists (
      select 1
      from public.share_sets s
      where s.id = share_set_id
        and s.organization_id = public.user_org_id()
        and s.revoked_at is null
        and (s.expires_at is null or s.expires_at > now())
    )
  );

drop policy if exists "share_set_items: parent write" on public.share_set_items;
create policy "share_set_items: parent write"
  on public.share_set_items for all
  using (
    exists (
      select 1
      from public.share_sets s
      where s.id = share_set_id
        and s.organization_id = public.user_org_id()
        and (s.created_by = auth.uid() or public.is_admin())
        and not public.is_viewer()
    )
  )
  with check (
    exists (
      select 1
      from public.share_sets s
      where s.id = share_set_id
        and s.organization_id = public.user_org_id()
        and (s.created_by = auth.uid() or public.is_admin())
        and not public.is_viewer()
    )
  );

create or replace function public.create_or_reuse_share_set(
  p_image_ids uuid[],
  p_expires_at timestamptz default null
)
returns table (
  share_set_id uuid,
  token text,
  expires_at timestamptz
)
language plpgsql
security definer
set search_path = public
as $$
declare
  v_org_id uuid := public.user_org_id();
  v_normalized_ids uuid[];
  v_fingerprint text;
  v_share_set_id uuid;
  v_token text;
  v_token_hash text;
  v_expires_at timestamptz := p_expires_at;
  v_bad_count int;
begin
  if auth.uid() is null then
    raise exception 'Not authenticated';
  end if;

  if public.is_viewer() then
    raise exception 'Viewer role cannot create share links';
  end if;

  select coalesce(array_agg(id order by id), '{}'::uuid[])
  into v_normalized_ids
  from (
    select distinct unnest(coalesce(p_image_ids, '{}'::uuid[])) as id
  ) dedup;

  if coalesce(array_length(v_normalized_ids, 1), 0) = 0 then
    raise exception 'No images provided';
  end if;

  -- All images must belong to caller org.
  select count(*)
  into v_bad_count
  from unnest(v_normalized_ids) as u(id)
  left join public.images i on i.id = u.id
  where i.id is null or i.organization_id <> v_org_id;

  if v_bad_count > 0 then
    raise exception 'Selection contains images outside your organization';
  end if;

  v_fingerprint := md5(array_to_string(v_normalized_ids, ','));
  v_token := 'ss_' || replace(gen_random_uuid()::text, '-', '');
  v_token_hash := md5(v_token);

  if v_expires_at is null then
    v_expires_at := now() + interval '7 days';
  end if;

  select s.id
  into v_share_set_id
  from public.share_sets s
  where s.organization_id = v_org_id
    and s.fingerprint = v_fingerprint
    and s.revoked_at is null
    and (s.expires_at is null or s.expires_at > now())
  order by s.created_at desc
  limit 1;

  if v_share_set_id is null then
    insert into public.share_sets (
      organization_id,
      created_by,
      token_hash,
      token_prefix,
      fingerprint,
      expires_at
    )
    values (
      v_org_id,
      auth.uid(),
      v_token_hash,
      left(v_token, 10),
      v_fingerprint,
      v_expires_at
    )
    returning id into v_share_set_id;
  else
    update public.share_sets
      set token_hash = v_token_hash,
          token_prefix = left(v_token, 10),
          expires_at = v_expires_at,
          revoked_at = null
    where id = v_share_set_id;

    delete from public.share_set_items where share_set_id = v_share_set_id;
  end if;

  insert into public.share_set_items (share_set_id, image_id, item_order)
  select
    v_share_set_id,
    ids.id,
    row_number() over (order by ids.id)
  from unnest(v_normalized_ids) ids(id)
  on conflict (share_set_id, image_id)
  do update set item_order = excluded.item_order;

  return query select v_share_set_id, v_token, v_expires_at;
end;
$$;

create or replace function public.resolve_share_set(
  p_token text
)
returns table (
  share_set_id uuid,
  image_id uuid,
  item_order int
)
language sql
stable
security definer
set search_path = public
as $$
  with target as (
    select s.id
    from public.share_sets s
    where s.organization_id = public.user_org_id()
      and s.token_hash = md5(p_token)
      and s.revoked_at is null
      and (s.expires_at is null or s.expires_at > now())
    limit 1
  )
  select i.share_set_id, i.image_id, i.item_order
  from public.share_set_items i
  join target t on t.id = i.share_set_id
  order by i.item_order;
$$;

grant execute on function public.create_or_reuse_share_set(uuid[], timestamptz) to authenticated;
grant execute on function public.resolve_share_set(text) to authenticated;
