-- =============================================================================
-- Share-set token hashing hardening (MD5 -> SHA-256, backward compatible)
-- =============================================================================

alter table public.share_sets
  add column if not exists token_hash_algo text not null default 'md5'
  check (token_hash_algo in ('md5', 'sha256'));

-- Existing rows were generated using md5(token).
update public.share_sets
set token_hash_algo = 'md5'
where token_hash_algo is distinct from 'md5';

-- New rows should default to sha256.
alter table public.share_sets
  alter column token_hash_algo set default 'sha256';

-- Keep existing index shape for lookup speed.
create index if not exists idx_share_sets_lookup_active_algo
  on public.share_sets (organization_id, token_hash_algo, token_hash)
