...
5. **Before creating any new Angular component, consult the component registry:** `docs/specs/component/registry.md` (index) and the generated `docs/specs/component/registry.*.supplement.md` slices (selector/variant tables). If the required component or variant exists, use it. If a variant is missing, flag it and ask. Do not implement inline HTML patterns that duplicate a registered component. The catalog's source is `docs/specs/component/registry.json` — edit that, never a supplement, then run `node scripts/generate-component-registry.mjs`. A new component under `apps/web/src/app/shared/` must be registered in the same change: `node scripts/check-component-registry.mjs` fails without an entry, and also fails on a dead path, a missing spec, a mismatched `specId`, or a stale supplement.
...
- **Reasoning with evidence grades (investigations, findings registers, correction plans)** — [`docs/study/README.md`](docs/study/README.md): every claim carries `[A]`–`[D]`, every file a `status`. **New reasoning goes here and nowhere else** (`docs/audits/`, `docs/backlog/`, `docs/implementation-blueprints/` are closed). Before resuming work on a subsystem, check for a study on it — for the upload pipeline that is [STUDY-005](docs/study/005-upload-pipeline-trace-findings.md) (what is broken) and [STUDY-006](docs/study/006-upload-pipeline-correction-plan.md) (decisions, the phase plan, and § Phase 5 — what is still open)
