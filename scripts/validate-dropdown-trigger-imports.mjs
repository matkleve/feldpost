#!/usr/bin/env node
/**
 * Validate relative imports for dropdown-trigger tree and external consumers.
 * Run from repository root: node scripts/validate-dropdown-trigger-imports.mjs
 */
...
const DROPDOWN_ROOT = path.join(WEB_SRC, 'app/shared/dropdown-trigger');
...
function referencesDropdownTrigger(content) {
  return content.includes('dropdown-trigger/');
}

function shouldValidate(file, content) {
  if (file.startsWith(DROPDOWN_ROOT + path.sep)) return true;
  return referencesDropdownTrigger(content);
}
