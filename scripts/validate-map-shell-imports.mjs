#!/usr/bin/env node
/**
 * Validate relative imports for map-shell tree and known external consumers.
 * Run from repository root: node scripts/validate-map-shell-imports.mjs
 */
...
const MAP_SHELL_ROOT = path.join(WEB_SRC, 'app/features/map/map-shell');
const EXTERNAL_CONSUMERS = [
  path.join(WEB_SRC, 'app/layout/authenticated-app-layout.component.ts'),
  path.join(WEB_SRC, 'app/features/upload/upload-panel/upload-panel-dialog-handlers.service.ts'),
  path.join(WEB_SRC, 'app/features/upload/upload-panel/upload-panel-dialog-actions.service.ts'),
  path.join(WEB_SRC, 'app/features/upload/upload-panel/upload-panel-job-file-actions.service.ts'),
];
...
function shouldValidate(file) {
  if (file.startsWith(MAP_SHELL_ROOT + path.sep)) return true;
