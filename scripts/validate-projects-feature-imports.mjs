#!/usr/bin/env node
/**
 * Validate relative imports for projects feature tree and known external consumers.
 * Run from repository root: node scripts/validate-projects-feature-imports.mjs
 */
...
const PROJECTS_ROOT = path.join(WEB_SRC, 'app/features/projects');
const EXTERNAL_CONSUMERS = [
  path.join(WEB_SRC, 'app/layout/authenticated-app.routes.ts'),
];
...
function shouldValidate(file) {
  if (file.startsWith(PROJECTS_ROOT + path.sep)) return true;
