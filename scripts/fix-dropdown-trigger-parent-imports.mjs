#!/usr/bin/env node
/**
 * After dropdown-trigger subfolder move: deepen broken relative imports.
 * Run from repository root: node scripts/fix-dropdown-trigger-parent-imports.mjs
 */
...
function dropdownDepth(file) {
  const rel = path.relative(DROPDOWN_ROOT, file);
...
function isOutsideDropdown(resolved) {
  return !resolved.startsWith(DROPDOWN_ROOT + path.sep);
}

function fixTsFile(file) {
  const depth = dropdownDepth(file);
  ...
  content = content.replace(importRe, (match, q1, p1, q1e, q2, p2, q2e) => {
    ...
    for (let extra = 1; extra <= depth + 3; extra++) {
      ...
      if (!isOutsideDropdown(resolved)) continue;
      changed = true;
...
const SCSS_PARTIALS_IN_SCSS_FOLDER = new Set([
  'dropdown-panel-flex-chain',
  'toolbar-dropdown-list-scroll',
...
function fixScssFile(file) {
  ...
  content = content.replace(useRe, (match, q1, imp, q3) => {
    ...
    const partialName = path.basename(imp).replace(/^_/, '');
    if (depth > 0 && SCSS_PARTIALS_IN_SCSS_FOLDER.has(partialName)) {
      const candidate = `${'../'.repeat(depth)}scss/${partialName}`;
      if (resolveScssImport(file, candidate)) {
        changed = true;
...
function main() {
  const files = listTsFiles(DROPDOWN_ROOT);
  let total = 0;
  let pass = 0;
  let round = 0;
  do {
  ...
  let scssFixed = 0;
  for (const file of listScssFiles(DROPDOWN_ROOT)) {
