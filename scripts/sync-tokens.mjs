...
if (!rootBlock) {
  console.error('sync-tokens: ERROR — could not locate :root { } block in _legacy-design-tokens.scss');
  process.exit(1);
}

const darkBlock = extractBlock(source, /@mixin\s+dark-theme-overrides\s*\{/);
if (!darkBlock) {
  console.error('sync-tokens: ERROR — could not locate @mixin dark-theme-overrides { } block in _legacy-design-tokens.scss');
  process.exit(1);
}
...
const output = {
  ...
  $metadata: {
    ...
    exportedFrom: 'apps/web/src/styles/_legacy-design-tokens.scss',
