#!/usr/bin/env node
/**
 * After map-shell subfolder move: deepen broken relative imports to core/, shared/, features/.
 * Run from repository root: node scripts/fix-map-shell-parent-imports.mjs
 */
...
function mapShellDepth(file) {
  const rel = path.relative(MAP_SHELL_ROOT, file);
...
function isOutsideMapShell(resolved) {
  return !resolved.startsWith(MAP_SHELL_ROOT + path.sep);
}

function fixFile(file) {
  const depth = mapShellDepth(file);
  ...
  content = content.replace(importRe, (match, q1, p1, q1e, q2, p2, q2e) => {
    ...
    for (let extra = 1; extra <= depth + 3; extra++) {
      ...
      if (!isOutsideMapShell(resolved)) continue;
      changed = true;
...
function main() {
  const files = listTsFiles(MAP_SHELL_ROOT);
  let total = 0;
  let pass = 0;
  let round = 0;
  do {
  ...
  let scssFixed = 0;
  for (const file of listScssFiles(MAP_SHELL_ROOT)) {
