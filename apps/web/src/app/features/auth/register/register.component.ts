...
export class RegisterComponent {
  ...
  constructor() {
    const inviteFromQuery = this.route.snapshot.queryParamMap.get('invite')?.trim() ?? null;
    if (inviteFromQuery) {
      this.form.controls.inviteCode.setValue(inviteFromQuery);
    }
  }
  ...
  protected async submit(): Promise<void> {
    ...
    const { fullName, email, inviteCode, password } = this.form.getRawValue();
    const { error } = await this.auth.signUp(email, password, fullName, inviteCode);
