...
import { BehaviorSubject, Observable, Subject, Subscription, catchError, from, map, of } from 'rxjs';
...
const RECENT_SEARCHES_STORAGE_KEY = 'geosite-recent-searches';
...
export class SearchBarComponent implements OnInit, OnDestroy {
    ...
    private applySearchResult(result: SearchResultSet): void {
        ...
        const commandSection =
            result.sections.find((section) => section.family === 'command') ?? null;
        ...
        switch (commitAction.type) {
            ...
            case 'open-content':
                ...
                void this.router.navigate(
                    [candidate.contentType === 'group' ? '/groups' : '/photos'],
                    {
                        queryParams: {
    ...
    private async fetchDbContentCandidates(query: string): Promise<SearchContentCandidate[]> {
        ...
        const projectCandidates = (projectsResponse.error || !Array.isArray(projectsResponse.data)
            ? []
        ...
        const groupCandidates = (groupsResponse.error || !Array.isArray(groupsResponse.data)
            ? []
