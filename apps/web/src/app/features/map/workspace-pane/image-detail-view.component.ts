...
export class ImageDetailViewComponent implements OnDestroy {
    ...
    async saveMetadata(entry: MetadataEntry, newValue: string): Promise<void> {
        ...
        this.metadata.update((list) =>
            list.map((m) =>
                m.metadataKeyId === entry.metadataKeyId ? { ...m, value: newValue } : m,
            ),
        );
    ...
    /** Action #6 — "Add to group" (placeholder — group picker from group-tab-bar spec). */
    openGroupPicker(): void {
        // Group picker will be integrated when GroupTabBarComponent is implemented.
    ...
    copyCoordinates(): void {
        ...
        navigator.clipboard.writeText(text).catch(() => {/* silent — clipboard may be unavailable */ });
        this.showContextMenu.set(false);
    }
