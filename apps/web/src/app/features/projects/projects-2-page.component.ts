...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, SearchBarComponent, GroupHeaderComponent, Projects2ToolbarComponent],
  template: `
    <main class="projects2-page">
      <section class="projects2-rail">
        <header class="projects2-header">
          <div class="projects2-header__title-wrap">
            <h1 class="projects2-header__title">{{ t('nav.item.projects', 'Projects') }} 2</h1>
            <p class="projects2-header__count">{{ projectCountLabel() }}</p>
          </div>

          <section
            class="projects2-search"
        ...
        @if (loading()) {
          ...
          <section class="projects2-empty">
            ...
            <p>{{ t('projects.page.empty.body', 'Try another search or reset your status filter.') }}</p>
          </section>
        } @else {
          <section class="projects2-content" [class.projects2-content--cards]="viewMode() === 'cards'">
                ...
                @if (viewMode() === 'list') {
                  <div class="projects2-list">
                    @for (project of section.projects; track project.id) {
                      <article class="project2-row" [style.--project-item-color]="colorTokenFor(project.colorKey)">
                        <span class="project2-row__dot" [style.background]="colorTokenFor(project.colorKey)"></span>
                        <div class="project2-row__content">
                          <h3 class="project2-row__name">{{ project.name }}</h3>
                          <p class="project2-row__meta">
                            {{ project.totalImageCount }} {{ t('projects.page.metric.photos', 'photos') }} ·
                  ...
                  <div class="projects2-grid">
                    @for (project of section.projects; track project.id) {
                      <article class="project2-card" [style.--project-item-color]="colorTokenFor(project.colorKey)">
                        <div class="project2-card__header">
                          <span class="project2-card__dot" [style.background]="colorTokenFor(project.colorKey)"></span>
                          <h3 class="project2-card__name">{{ project.name }}</h3>
                        </div>
                        <p class="project2-card__badge">
                          {{ project.totalImageCount }} {{ t('projects.page.metric.photos', 'photos') }}
                        </p>
                        <p class="project2-card__meta">{{ formatRelativeDate(project.lastActivity) }}</p>
                      </article>
                    }
                  </div>
                }
  ...
  styles: [
    `
      ...
      .projects2-header {
        display: grid;
        grid-template-columns: minmax(0, 1fr) minmax(18rem, 36rem) auto;
        gap: var(--spacing-3);
        align-items: center;
      }
      ...
      .projects2-search ss-search-bar {
        position: static;
        transform: none;
        left: auto;
        top: auto;
...
export class Projects2PageComponent {
  ...
  readonly groupingOptions = computed(() => GROUPING_OPTIONS.map((option) => ({
    ...option,
    label: this.projectLabel(option.id, option.label),
  })));
  readonly filterOptions = computed(() => FILTER_OPTIONS.map((option) => ({
    ...option,
    label: this.projectLabel(option.id, option.label),
  })));
  readonly sortOptions = computed(() => SORT_OPTIONS.map((option) => ({
    ...option,
    label: this.projectLabel(option.id, option.label),
  })));
  readonly hasArchivedProjects = computed(() => this.projects().some((project) => project.status === 'archived'));
  ...
  readonly visibleProjects = computed(() => {
    ...
    const searchScoped = this.projects()
      .map((project) => ({
        ...project,
        matchingImageCount: query ? (counts[project.id] ?? 0) : project.totalImageCount,
      }))
      .filter((project) => (status === 'all' ? true : project.status === status))
      .filter((project) => (query ? (counts[project.id] ?? 0) > 0 : true));

    const filterScoped = this.applyProjectFilters(searchScoped);
  ...
  readonly groupedSections = computed<ProjectGroupedSection[]>(() => {
    ...
    if (groupings.length === 0) {
      return [{ id: 'all-projects', heading: '', level: 0, projectCount: projects.length, projects }];
    }
  ...
  readonly projectCountLabel = computed(() => {
    ...
    return this.t('projects.page.count.multi', '{count} projects').replace('{count}', String(total));
  });

  constructor() {
    effect(() => {
      const labelsById = new Map(this.groupingOptions().map((option) => [option.id, option.label]));
      this.activeGroupings.update((current) => current.map((entry) => ({ ...entry, label: labelsById.get(entry.id) ?? entry.label })));
    });
  ...
  formatRelativeDate(value: string | null): string {
    ...
    if (days < 30) {
      return this.t('projects.page.relative.daysAgo', '{count} days ago').replace('{count}', String(days));
    }
    const months = Math.floor(days / 30);
    if (months < 12) {
      return this.t(months === 1 ? 'projects.page.relative.monthAgo.single' : 'projects.page.relative.monthAgo.multi', months === 1 ? '1 month ago' : '{count} months ago').replace('{count}', String(months));
    }
    const years = Math.floor(months / 12);
    return this.t(years === 1 ? 'projects.page.relative.yearAgo.single' : 'projects.page.relative.yearAgo.multi', years === 1 ? '1 year ago' : '{count} years ago').replace('{count}', String(years));
  }

  private async refreshSearchCounts(): Promise<void> {
    const counts = await this.projectsService.loadGroupedSearchCounts(this.searchTerm(), this.statusFilter());
  ...
  private matchesProject(project: ProjectListItem, rules: Array<{ conjunction: string; property: string; operator: string; value: string }>): boolean {
  ...
  private evaluateRule(project: ProjectListItem, rule: { property: string; operator: string; value: string }): boolean {
    ...
    switch (rule.operator) {
      case 'contains': return fieldStr.includes(ruleValue);
      case 'equals':
      case 'is': return fieldStr === ruleValue;
      case 'is not': return fieldStr !== ruleValue;
      case 'before': return fieldStr < ruleValue;
      case 'after': return fieldStr > ruleValue;
      default: return true;
    }
  }
  ...
  private getProjectFieldValue(project: ProjectListItem, property: string): string | number | null {
    switch (property) {
      ...
      case 'district': return project.district;
      case 'city': return project.city;
      case 'color-key': return project.colorKey;
      case 'image-count': return project.totalImageCount;
      case 'updated-at': return project.updatedAt;
      case 'last-activity': return project.lastActivity;
      default: return null;
    }
  }
  ...
  private buildGroupedSections(projects: ProjectListItem[], groupings: GroupingProperty[], pathKeys: string[]): ProjectGroupedSection[] {
    if (groupings.length === 0) {
      return [{ id: pathKeys.join('||') || 'all-projects', heading: '', level: Math.max(pathKeys.length - 1, 0), projectCount: projects.length, projects }];
    }
    ...
    for (const project of projects) {
      ...
      if (bucket) bucket.push(project); else buckets.set(value, [project]);
    }
  ...
  private getGroupingValue(project: ProjectListItem, groupingId: string): string {
    switch (groupingId) {
      case 'status':
        return project.status === 'archived' ? this.t('projects.page.status.archived', 'Archived') : this.t('projects.page.status.active', 'Active');
      case 'district':
        return project.district ?? this.t('projects.page.value.unknownDistrict', 'Unknown district');
      case 'city':
