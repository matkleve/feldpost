...
**Written:** 2026-09-24. **On:** `cursor/addable-apps-change-plan-cb0a`, after STUDY-016 through STUDY-020 and `20260924120000_widget_install.sql`. The owner said the studies had not critiqued the decisions. This file is that critique. **Status `proposed`.** It does not revoke the spec.

STUDY-016 through STUDY-020 grade owner sentences as `[D]` and measure the code those sentences hit. They do not say where a decision cannot be true, or where a later commit dropped one. That is the gap.

## 1. “Unless the organization turned them off” has no input

The decision says existing apps stay, and come off only if the organization turned them off. `[D]` STUDY-017 current record.
...
The decision says a person signs up without an invite, logs in, adds the Organization widget, and creates the organization there. The same email may belong to several organizations. `[D]` STUDY-017 current record.
...
Install is per user. The organization may also lock a widget on, and a push turns it back on for people who turned it off. `[D]` STUDY-017 current record.
...
Map is supposed to be the same kind of module as Media. `[D]` STUDY-016. The layout still always contains `app-map-shell`. `[A]` STUDY-019. The nav hides a row. The route is still there. Uninstall, so far, is a label.

A shared link was supposed to open a widget until logout and then be gone. `[D]` STUDY-017. The spec now says opening the path does not write a row. `[A]` `docs/specs/system/widget-install.md` Actions row 8. Nothing blocks the route, before logout or after. Hiding the nav row is not “gone after logout.” The URL still opens the page.

## 5. Chat as its own widget was dropped

Chat is its own widget, and its rows stay when it is removed. `[D]` STUDY-017. The migration’s widget check list is `map`, `media`, `projects`, `colleagues`, `organization`. `[A]` `20260924120000_widget_install.sql`. There is no `chat` id. Colleagues remains the chat screen.
