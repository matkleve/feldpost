...
| STUDY-014 | [Where a System One model would fit in Feldpost — and where it would not](./014-where-a-system-one-model-would-fit.md) | analysis | `proposed` | Answers the question STUDY-012 could not: not *is Jev good*, but *where in our own repo would a typed-decision model make sense*. Inventories 25+ closed-set decisions in `core/` — most are state, config or correct rules, with **no** fit — and finds three candidates, all links of the path→address chain. Measured: the classifier is **better than expected** (0 of 30 trade words misread as a street), so the cost is **silence, not error**, and the case is coverage rather than correctness. Also measures a new defect: `Baustelle Seestadtstrasse 5` yields street `Baustelle Seestadtstrasse`, splitting one building into two grouping keys and two tray questions. Recommends calibrating the existing classifier first — in-house, zero cost, nobody has done it — and puts Constitution § 1 data egress ahead of any quality question. |
