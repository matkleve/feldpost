...
| STUDY-008 | [Chunking classification without splitting what belongs together](./008-classification-chunking-strategy.md) | investigation | `proposed` | How to chunk `classifyBatch` (Phase 3.3). Measures that reading structure is ~2 900× cheaper than classifying it (0.17 s vs 8.2 min at 100 000 files), and finds that a split chunk boundary only costs a duplicate question when the user answers a tray before its remaining members arrive — so deferring tray activation may matter more than where the cut falls. |
| STUDY-009 | [Tray question budget and priority](./009-tray-question-budget-and-priority.md) | proposal | `decided` | Owner's proposal to prioritise tray questions and suppress low-priority ones on large batches. Separates it into priority (cheap, static, buildable now), budget (needs a measurement that does not exist) and a deferred-improvement surface. Finds that the budget must count **questions after merge, not files** — 10 000 files in one folder is one question — that "20 m" is a leaky proxy for stakes where question *kind* is not, that `presentationBundleMaxDialogueUnits` is dead config that looks like an existing budget, and that suppression without a visible backlog is TRAP-021's shape. **Measured 2026-09-21 (#229): the worst of five corpus shapes asks 29 questions of 10 000 files and every one is `critical`, so the budget was closed as not planned — priority (#231) and the backlog surface (#232) shipped.** |
| STUDY-012 | [Shell grid layout — change plan (owner mock 2026-09-22)](./012-shell-grid-layout-change-plan.md) | proposal | `proposed` | Four-track shell (left rail · map · content panels · right rail). **Planning — [#257](https://github.com/matkleve/feldpost/issues/257).** |
| STUDY-013 | [Widget modularity platform — change plan](./013-widget-modularity-platform-change-plan.md) | proposal | `proposed` | [#258](https://github.com/matkleve/feldpost/issues/258). **After #257.** |
| STUDY-014 | [Org + Colleagues widget bundle (bundle-only)](./014-org-colleagues-widget-bundle.md) | investigation | `proposed` | Catalog + manifest model for team bundle. |
| STUDY-015 | [Map as immutable platform core](./015-map-as-platform-core.md) | investigation | `proposed` | Map not a catalog widget; rail placement options. |
| STUDY-016 | [Database modularization for widgets](./016-database-modularization-for-widgets.md) | investigation | `proposed` | Install registry + RLS tagging; modular DB work. |
