...
| **Addable apps (widgets)** ([#258](https://github.com/matkleve/feldpost/issues/258)) | [STUDY-016](./016-addable-apps-repo-shape.md), [STUDY-017](./017-addable-apps-change-plan.md) | 016 is the definition. 017 is the change plan. Both `proposed`. Do not add tables from either until the owner answers 017’s questions and a spec exists. |
...
| STUDY-017 | [Addable apps — change plan](./017-addable-apps-change-plan.md) | proposal | `proposed` | Phased plan for per-user install, organization preinstall or allow, and Map as a widget. Address and GPS stay on the subject. Questions block the schema. |
