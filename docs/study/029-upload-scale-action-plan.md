...
**Written:** 2026-09-23 · **From:** [STUDY-028](./028-upload-flow-scale-and-folder-shapes.md), product commit `54e7f192` · **How:** the same headless harness, plus one re-run that printed the single `upload_error`. No product code in this change. Renumbered from STUDY-017, then from STUDY-021, on 2026-09-24; those ids belong to other studies. The two graded sentences that still say STUDY-016 mean this measurement, now STUDY-028.
...
Verify: the locality probe from STUDY-028 § 2, same seed, those three folders come back with city and postcode. A one-word city (`Wien`, `Linz`, `Graz`) does not change.
...
Verify: a required `company_street` run at 10 000 settles or names the phase that is still moving, and a lane of more than the visible window does not mount every row. The DOM part was not measured in a browser (STUDY-028 § 5).
