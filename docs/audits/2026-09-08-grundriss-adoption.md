...
*Landed:* wired into `verify` as a **soft** check (reports, does not fail), and the false "green (0 errors, 0 warnings)" line in `docs/specs/SPEC-SIZE-BACKLOG.md` replaced with the real numbers and a no-new-debt rule.

*And it is not alone.* The first full `verify` run showed **three** checks red on `main`: `specs` (201 errors), `lint` (151 errors + 1068 warnings, with `--max-warnings 0`), and `test` — where the test bundle **does not compile**: ~101 TS errors including four `*.spec.ts` files importing paths that do not exist. `ng build` is green, so this is test-only. It survived because the job that would catch it has been red and unattended: the last eight `Build & Test` runs on `main` (2026-06-23 → 2026-07-01) all concluded `failure`, each in about five seconds. All three enter the gate soft, with the measured count as a ratchet that may only go down. *Still open:* the broken test bundle first, then the lint and spec counts, then promote each check to hard.
