...
| **UP-26** | The **false-hub pattern is gone**: six location services (`placement`, `source-conflict`, `tray-flow`, `candidate-apply`, `pre-resolve-orchestrator`, `disambiguation-registration`) no longer import `UploadLocationResolutionService` purely to reach a sibling method the facade only forwarded. The two event subjects it actually owned (`disambiguationRequired$`/`disambiguationResolved$`) moved to the already-leaf `UploadLocationDisambiguationStoreService`; the facade's public API is unchanged (still delegates). A regression test (`upload-location-dependency-cycle.spec.ts`) asserts the six files never re-import the facade. `madge --circular src/app/core/upload` dropped from 27 to 24 cycles. | Two **genuine, documented** mutual dependencies remain by design (`tray-flow` ↔ `candidate-apply`/`pre-resolve-orchestrator`, `placement` ↔ `pre-resolve-orchestrator` — each pair already depends on the other one direction, so `Injector.get()` is used deliberately, not as a hub workaround). Two **separate, pre-existing** issues are untouched and out of this pass's scope: the tray-producer-adapter ↔ `disambiguation-registration` reverse-layering cycle (`core/upload/location/upload-location-disambiguation-registration.service.ts:120-124`), and `UploadManagerService`'s own entanglement with the location layer via `candidate-apply`'s `kickQueueAfterLocationGate()` call. UP-27 (DB access not behind adapters, 4 unconsolidated `*.types.ts`) is untouched. |
