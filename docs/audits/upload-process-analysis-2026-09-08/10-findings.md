...
| **UP-30** | medium | S | spec | `upload-manager.md:139` types `issueKind` with 4 members (code has 8) and names `duplicate_photo` first — the one member with no write site. | `05-spec-drift.md` C4 | — | **Table synced 2026-09-09 (P3)**: `upload-manager.md:139` now lists the real 8-member union. Deleting the dead `duplicate_photo` member itself is coordinated with P4 (dead-code deletion) — **not started**, so it is still listed. |
