...
**F4 — Four fire-and-forget calls on the happy path; three are unguarded (refined in Phase 7).**
`…/upload-file-persist.util.ts:211-220` (reverse geocode), `…/upload-new-run-upload-phase.util.ts:237-246` (dedup hash insert), `…/upload-new-post-save.util.ts:240` (`void persistMismatch(...)`), `…/upload-new-post-save.util.ts:292` (`void persistThumbnail(...)`).

Of the four, only the reverse geocode is internally guarded: `core/upload/address-resolution/upload-address-resolve.util.ts:20-51` wraps everything in `try/catch` and falls back to `markUploadLocationUnresolvable`. The other three can reject into a global unhandled rejection — in particular `core/upload/support/upload-db-postwrite.util.ts:38` ends the insert with a bare `.then()` and **no rejection handler**. Details and consequences in `07-failure-modes.md`. Answers plan § 3 Q9 for this path: **the failure is silent either way** — a lost dedup hash, a lost mismatch value or a lost thumbnail produces no user-visible signal.
...
**F9 — The storage path extension is unsanitised, user-controlled input (severity narrowed in Phase 8).**
`…/upload-file-persist.util.ts:83-84`: `const ext = (input.file.name.split('.').pop() ?? 'jpg').toLowerCase();` then `${orgId}/${user.id}/${uuid}.${ext}`. Nothing strips `/`, `..`, whitespace or control characters, and `supabase/AGENTS.md` states the frontend is untrusted.

Phase 8 § 6 read the storage policy and settled the impact: `"media: org members can upload"` requires `foldername(name)[1] = user_org_id()` **and** `[2] = auth.uid()`, so a crafted extension still lands inside the caller's own prefix and **no cross-tenant write is possible**. What remains is a malformed key inside the caller's own namespace, which can confuse later path parsing (`organizationIdFromStoragePath`). **Revised severity `low`**, effort `S`. Whether Supabase Storage normalises `..` segments server-side is still `unverified`.
