...
**F10 — `statusLabel` is hardcoded English.**
`apps/web/src/app/core/upload/support/upload-job-state.service.ts:56-99` returns literals ("Queued", "Reading EXIF…", "Uploading…", "Uploaded"); `…/upload-new-prepare-route.util.ts:205` writes `'Choose location or project'` / `'Missing location'`; `core/upload/location/upload-location-disambiguation-registration.service.ts:100` writes `'Choose address'`. `statusLabel` is rendered directly at `features/upload/upload-panel/upload-panel-item.component.html:99,102,104` through `statusLabelText()` (`upload-panel-item-helpers.ts:72-106`), whose fallback at `:106` is `job.statusLabel`. `npm run i18n:guard` does not catch these because it only scans templates for literal text nodes (`00-baseline.md` § 2, B7). Partial answer to plan § 3 Q10; the full sweep is Phase 3/6.
