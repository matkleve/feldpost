...
| **toggle-group** item | 17 | ✓ | ✓ on/off | ✓ | ✓ | complete, but `transition-all` + 9 inline `color-mix` |
| **form-field** | 15 | n/a | n/a | n/a | n/a | layout only — fine |
| **dialog** | 7 | n/a | n/a | n/a | n/a | fine, except a **raw `bg-black/80`** scrim |
| **tabs** trigger | 5 | ✓ | ✓ active | ✓ | ✓ | complete, but `transition-all` + 5 inline `color-mix` |
| **badge** | 4 | · | · | · | · | has a `focus:` ring and **nothing else** — see § 4.1 |
| **select** | 2 | · | · | · **`focus:` only** | ✓ | ring stays after a mouse click — see § 4.1 |
...
| **card** | **0** | — | — | — | — | 🔴 **dead** — no consumer anywhere |
| **toast** | **0** | — | — | — | — | 🔴 **dead** — no consumer anywhere; also `transition-all` |
...
### 4.1 `focus:` where it should be `focus-visible:` — badge and select
...
which breaks keyboard users. Two-character fix in two files.
...
is dead code). Both readings are defects; the spec has to say which.
...
### 4.5 `transition-all` in three primitives — tabs, toast, toggle-group
...
`transition-colors`. Three call sites.

### 4.6 Two dead primitives
...
is explicit that unused exports go. Either they are the target of a migration
nobody finished, in which case that belongs in `docs/migration/`, or they are
leftovers. Toast in particular is misleading: there *is* a working toast system
(`shared/toast/`), so a reader finds two and has to work out which is real.

### 4.7 Smaller items

- **`dialog`: `bg-black/80`** is the only raw colour in the entire primitive
  layer. A scrim token would also let the sandstone theme warm it.
- **`dialog`: `duration-200`** literal instead of `--motion-duration-slow`.
- **`popover`: `outline-none`** on a panel with no focus replacement — the § X2
  pattern, in a primitive.
- **`toggle-group`: `motion-reduce:` utilities** are now redundant with the
  global rule from § M1 and can go when the file is next touched.
...
| --- | --- | :---: | :---: | --- |
| P-1 | `focus:` → `focus-visible:` in badge and select | **P0** | S | ring after every mouse click; two characters, two files |
...
| P-4 | `transition-all` → named properties in tabs, toast, toggle-group | **P1** | S | 3 call sites |
| P-5 | Delete or document `shared/ui/card` and `shared/ui/toast` | **P1** | S | 10 dead files; toast has a live namesake |
| P-6 | Scrim token for `bg-black/80`; motion token for `duration-200` | **P1** | S | the only raw colour in the layer |
...
| P-11 | Drop the redundant `motion-reduce:` utilities in toggle-group | **P3** | S | superseded by the global rule |
