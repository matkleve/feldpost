...
- Prefer **`var(--motion-duration-fast) var(--motion-ease-out)`** (same timing as **`--transition-fade-in`** after Phase 7 Batch 31 removed the **`--transition-interactive`** alias) for direct user-driven state changes.
