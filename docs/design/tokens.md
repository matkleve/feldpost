...
| ---------------------------- | ----------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `.ui-container`              | Shared panel shell for sidebar, search surfaces, upload panel, and similar overlays | Panel radius `--container-radius-panel`, token-driven padding, token-driven gap             | Defines the outer geometry boundary. Components may override padding/gap with CSS variables, but should keep the same corner radius across state changes. |
| `.ui-container--panel-tight` | Tight panel inset for stabilized reference surfaces                                 | Tight inline/block padding and tight internal gap                                           | Use for the Sidebar and Search Bar so both surfaces share the same alignment boundary before expanding the pattern elsewhere.                             |
...
- Sidebar and Search Bar should share the same tight panel inset tokens so row alignment starts from the same container boundary.
- If a pill treatment causes transition instability, keep the standard panel radius in all states.
- Visual state changes may affect color, opacity, clipping, and outer container width. They must not change row geometry.
