...
.page-grid {
  display: grid;
  grid-template-columns: repeat(auto-fill, minmax(128px, 1fr)); // Adjust for list/card
  gap: var(--spacing-3);
  list-style: none;
  margin: 0;
  padding: 0;
...
```typescript
import { PageContainerComponent, VStackComponent, HStackComponent } from '../../shared/containers';
```
...
8. **Use i18n keys** for all user-visible text (media.page.*, common.*)
