...
| --- | --- | --- |
| Audit scope to issues | `.github/skills/audit-scope-to-issues/SKILL.md` | Comprehensively audit a folder/file for spec drift, bugs, and naming drift, then create GitHub issues from confirmed findings with type and priority labels and report unclear findings separately. |
| Verify issues | `.github/skills/verify-issues/SKILL.md` | Re-check open issues against code/spec evidence, verify priority-label metadata, close only complete issues, and comment on incomplete or unverifiable issues. |
