...
| **Explicit precision metadata** | `locations.address_precision` stores the highest established tier using Search Object / `groupingKey` vocabulary: `country` \| `state` \| `postcode` \| `city` \| `street` \| `houseNumber`. Migration `20260910140000_upload_address_precision.sql` (**verified locally** 2026-09-12 via `scripts/local-verify/`; hosted apply outstanding — #136, #137). |
...
- [x] Bundle caps: 5 s max window, 5 dialogue units max; 1A+1B = one unit — `PRESENTATION_BUNDLE_WINDOW_MS=5000` and `PRESENTATION_BUNDLE_MAX_DIALOGUE_UNITS=5` in `upload-resolver-tray-orchestrator.types.ts`, read by `upload-resolver-tray-orchestrator.service.ts`; shared `dialogueUnitId` via `dialogueUnitIdForGroup` in `adapters/upload-location-tray-producer.adapter.ts`. The cap is **not** configurable per organization — an unread `UploadLocationConfig` field that suggested it was is gone ([#230](https://github.com/matkleve/feldpost/issues/230)).
