...
| 5 | When the exact hits **disagree** on the country, **no country is derived** — but every hit is still written: identical values collapse to one, differing ones land as two entries at that level, where `adminLevelConflicts` opens the tray that asks. Ambiguity is surfaced, never guessed. |
