...
The same applies to the **server same-user** match (resume / prior session):
the current file's address is attached to the matched media. There the existing
address is not known client-side, so the attach is unconditional but **cheap and
idempotent** (`addLocation`, no geocode; location dedupe key + unique link make a
same-address resume a DB no-op).
