...
| 7c  | Same file picked twice in one batch (double folder pick)         | Skips the later byte-identical sibling deterministically, before placement/tray, via a client per-batch hash registry — no second server check | Single early gate; see [dedup-scope supplement](./upload-manager-pipeline.dedup-scope.supplement.md) |
