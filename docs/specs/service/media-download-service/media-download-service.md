...
- Runtime ownership target (service facade + adapters):
  ...
  - `apps/web/src/app/core/media-download/adapters/edge-export-orchestrator.adapter.ts` (ZIP export; edge-first with local fallback)
- Adapter spec split:
...
| Retired MediaOrchestratorService (removed from tree)                                                  | Former tier and fallback policy       | **Done:** `TierResolverAdapter` + `MediaDownloadService`                             |
| Retired ZipExportService and helpers (removed from tree)                                              | Former client ZIP orchestration       | **Done:** `EdgeExportOrchestratorAdapter` + `media-download.helpers.ts`              |
...
| `apps/web/src/app/features/map/map-shell/component/map-shell.component.ts`                                        | Marker preview and staleness consumer | Facade preview + staleness calls                 |
...
| `apps/web/src/app/core/media-download/adapters/edge-export-orchestrator.adapter.ts`                    | ZIP export orchestration (edge-first with local fallback)                           |
...
| `apps/web/src/app/features/map/map-shell/component/map-shell.component.ts`                                        | Marker signing, preload, staleness consumer                                         |
...
| Retired MediaOrchestratorService (removed from tree)                                                  | `apps/web/src/app/core/media-download/adapters/tier-resolver.adapter.ts`                                                                               | RETIRED          | Deprecated compatibility wrapper removed after all runtime consumers were migrated                                      |
...
| Retired ZipExportService (removed from tree)                                                            | `apps/web/src/app/core/media-download/adapters/edge-export-orchestrator.adapter.ts`                                                                    | RETIRED          | Deprecated compatibility wrapper removed after all runtime consumers were migrated                                      |
| Retired zip-export helpers (removed from tree)                                                        | `apps/web/src/app/core/media-download/media-download.helpers.ts` (shared module helpers)                                                               | RETIRED          | Helper ownership shifted into media-download module                                                                     |
...
| `apps/web/src/app/features/map/map-shell/component/map-shell.component.ts`                                        | Facade calls (`getSignedUrl`, `invalidateStale`, `preload`)                                                                                            | Done             | Direct typed dependency now points to `MediaDownloadService`                                                            |
| `apps/web/src/app/shared/media-item/media-item.component.ts`                                               | Facade calls (`getLoadState`, `getSignedUrl`, tier/file-type helpers)                                                                                  | Done             | Direct typed dependencies now point to `MediaDownloadService`                                                           |
