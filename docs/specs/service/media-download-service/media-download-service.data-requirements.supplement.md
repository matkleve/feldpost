...
| `MediaDownloadService.getState`               | `(mediaId: string, slotSizeRem: number) => Observable<MediaDisplayDeliveryState>`                                                                                                                                                            | Per-item delivery stream for display shells (subscribes to cache adapter events)                                     |
...
| `MediaDownloadService.setLocalUrl` / `injectLocalUrl` | `setLocalUrl` delegates to `injectLocalUrl`; both `(mediaId: string, blobUrl: string) => void`                                                                                                                                          | Attach/replace instant preview path (`setLocalUrl` preferred at callsites)                                    |
