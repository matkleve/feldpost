...
Desktop uses a compact popover anchored to the marker hit zone. Mobile uses a bottom action sheet with the same action order. The surface follows the shared dropdown shell: `--color-bg-elevated`, `1px` border (`--color-border`), `box-shadow: var(--shadow-lg)` (**tweakcn `:root`** — **Batch 39** removed duplicate **`--shadow-*`** rows from **`_legacy-design-tokens.scss`**; reference [`docs/design/tokens.md`](../../../design/tokens.md) §3.5), **`--radius-lg`** (**`styles.scss` `@theme inline`** — **Batch 44**, not the legacy bridge; see [`docs/migration/phase-7-token-migration.md`](../../../migration/phase-7-token-migration.md) §Batch 44). Action rows use `.dd-item` primitives with `2.75rem` (44px) minimum row height and `0.8125rem` labels. Destructive rows use `.dd-item--danger`.
