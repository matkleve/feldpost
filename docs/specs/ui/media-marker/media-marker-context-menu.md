...
Desktop uses a compact popover anchored to the marker hit zone. Mobile uses a bottom action sheet with the same action order. The surface follows the shared dropdown shell: **`--color-bg-elevated`**, **`1px solid --color-border`**, **`box-shadow: var(--shadow-lg)`**, **`--radius-lg`**. **Authors:** use [`docs/design/tokens.md`](../../../design/tokens.md) for semantic / tweakcn token names and [`docs/specs/component/filters/dropdown-system.md`](../../component/filters/dropdown-system.md) for `dd-*` shell contracts — not migration batch notes as the source of variable names. Action rows use `.dd-item` primitives with `2.75rem` (44px) minimum row height and `0.8125rem` labels. Destructive rows use `.dd-item--danger`.
