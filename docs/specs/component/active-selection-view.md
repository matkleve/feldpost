...
The composed content experience inside the Workspace Pane when the Active Selection tab is active. It is the hub where map exploration meets curation: media from a cluster click or radius selection are loaded, then filtered, sorted, grouped, and browsed. The user can inspect individual media items, apply Notion-style filter rules, group by any property, scope to projects, sort by any column, and ultimately save curated selections as persistent named groups.
...
**No-photos state** (marker clicked but RPC returns 0 rows): centered 📷 icon, "No media at this location", and a secondary hint "Media may not have been uploaded yet for this area."
...
| --- | ---------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ---------------------------- |
| 1   | Cluster click on map                     | Media items loaded via `cluster_images` RPC → pipeline processes → grid renders thumbnails                    | `rawImages` populated        |
| 2   | Radius selection on map                  | Media items loaded via viewport query (bounded) → pipeline processes → grid renders                           | `rawImages` populated        |
| 3   | Clicks "Grouping" toolbar button         | Grouping dropdown opens — drag/click to activate properties, media items regroup instantly                    | `activeGroupings` changed    |
...
flowchart LR
    ...
    subgraph Pipeline["Computed Signal Chain"]
        direction TB
        S1["1. Project Filter\nkeep where media item has membership in selectedProjectIds\n(skip if empty = all)"]
...
stateDiagram-v2
    ...
    state NoPhotos {
        [*]: RPC returned 0 rows
        [*]: Marker exists but no media items
        [*]: "No media at this location"
    }

    state BrowsingFlat {
        [*]: Media items loaded, no grouping
        [*]: Flat thumbnail grid
        [*]: Toolbar active
    }

    state BrowsingGrouped {
        [*]: Media items loaded + grouping active
        [*]: Group headers + grids per section
        [*]: Toolbar active
    }

    state FilteredEmpty {
        [*]: Media items loaded but all filtered out
        [*]: "No media items match" + Clear filters
    }

    state DetailView {
        [*]: Single media full-res view
        [*]: Toolbar hidden, back arrow visible
    }

    Empty --> Loading: Cluster click / radius select
    Loading --> BrowsingFlat: Media items received (no grouping)
    Loading --> BrowsingGrouped: Media items received (grouping was active)
    ...
    BrowsingFlat --> FilteredEmpty: Filters exclude all media items
    BrowsingGrouped --> FilteredEmpty: Filters exclude all media items
...
```mermaid
flowchart TD
    Click["User clicks a media marker"] --> GetState["Read markerState\n(count, lat, lng, mediaId)"]
    ...
    RPCResult -->|"rows > 0"| SetImages["rawImages.set(mediaItems)\nGrid renders thumbnails"]
    RPCResult -->|"rows = 0"| NoPhotos["Show 'No media at this location'\nplaceholder with hint"]
...
- [x] Cluster click loads media items via `cluster_images` RPC and populates the grid
- [x] Single marker click loads media items via `cluster_images` RPC and opens detail view
- [ ] Radius selection loads media items and populates the grid
- [x] Skeleton loading state shown during RPC call
- [x] No-photos placeholder shown when RPC returns 0 rows ("No media at this location")
...
- [x] **Filter**: adding rules immediately removes non-matching media items from the grid
...
- [x] **Projects**: checking/unchecking projects filters media items by project membership
...
- [ ] Clicking a thumbnail opens Media Detail View (replaces grid)
- [ ] Returning from detail view preserves scroll position and all toolbar state
- [x] Empty filter state: "No media items match the current filters" + "Clear filters" button
- [x] New cluster/radius selection replaces current media items and resets filters
- [x] Closing workspace pane clears Active Selection and resets all toolbar state
- [ ] Save as named group via "+" in Group Tab Bar persists media items to DB
- [x] Pipeline is 100% client-side (no server round-trips for filter/sort/group changes)
- [ ] Performance: re-render completes in < 100ms for 500 media items
