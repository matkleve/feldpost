...
The menu is a compact floating surface anchored near the pointer position, using the shared elevated menu shell and `dd-*` action rows. It uses `--color-bg-elevated`, `1px` border (`--color-border`), `box-shadow: var(--shadow-lg)` (**tweakcn `:root`** shadow ladder — **Batch 39** removed duplicate **`--shadow-*`** rows from **`_legacy-design-tokens.scss`**; see [`docs/design/tokens.md`](../../../design/tokens.md) §3.5), and **`--radius-lg`** (**`styles.scss` `@theme inline`** — **Batch 44**, not the legacy bridge; see [`docs/migration/phase-7-token-migration.md`](../../../migration/phase-7-token-migration.md) §Batch 44). Default width is `14rem` (224px) with item rows at `2.75rem` (44px) minimum touch target height. Items use leading icons (`1rem`) and labels (`0.8125rem`) with warm clay hover (`color-mix(in srgb, var(--color-clay) 8%, transparent)`). On narrow mobile viewports, the same actions render as a bottom action sheet instead of a tiny anchored popover. The app menu uses normal viewport clamping only; there is no special "mirror against browser menu" placement logic.
...
| --- | -------------------------------------------------------------- | --------- | ----------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| 0   | Active radius exists + right-click inside radius               | primary   | Radius Context Menu wins (project actions), map menu does not open                                          | radius hit-test precedence                                     |
| 0a  | Active radius exists + short right-click outside radius        | primary   | Radius closes immediately; map menu does not open on that same click                                        | outside-radius dismiss rule                                    |
| 1   | First short right-click on empty map (desktop)                 | primary   | Opens Map Context Menu at pointer coordinates and suppresses native browser menu                            | `MapAdapter` context event + no marker target                  |
| 2   | Second short right-click near same point within `2000ms`       | secondary | Closes app menu and allows native browser context menu (no `preventDefault`)                                | double-secondary-click handshake                               |
| 3   | Second right-click outside handshake window                    | secondary | Treated as a new first click; app menu re-anchors and native menu is suppressed                             | handshake reset                                                |
| 4   | Right-click and drag beyond movement threshold (`8px`)         | primary   | Cancels menu opening, starts Radius Selection interaction                                                   | Radius Selection gesture recognizer                            |
| 5   | Long-press on empty map without drag (mobile, `>= 380ms`)      | primary   | Opens action sheet variant of Map Context Menu                                                              | Touch long-press recognizer                                    |
| 6   | Long-press then drag beyond threshold (`8px`)                  | primary   | Suppresses menu and starts Radius Selection                                                                 | Radius Selection gesture recognizer                            |
| 7   | Selects `Media Marker hier erstellen`                          | primary   | Creates an ephemeral media-marker draft at clicked lat/lng, opens Workspace Pane, and focuses upload prompt | `MapShellComponent` + `WorkspacePane` + `UploadManagerService` |
| 8   | Selects `Hierhin zoomen (Hausnaehe)`                           | primary   | Centers clicked point and zooms to building-level context                                                   | `MapAdapter.setView(latlng, 19)`                               |
| 9   | Selects `Hierhin zoomen (Strassennaehe)`                       | primary   | Centers clicked point and zooms to street-level context                                                     | `MapAdapter.setView(latlng, 17)`                               |
| 10  | Selects `Adresse kopieren`                                     | secondary | Resolves human-readable address for clicked point and copies to clipboard                                   | `GeocodingService.reverse()` + clipboard + toast               |
| 11  | Selects `GPS kopieren`                                         | secondary | Copies `lat, lng` to clipboard and shows toast                                                              | `navigator.clipboard.writeText` + `ToastService`               |
| 12  | Selects `In Google Maps oeffnen`                               | secondary | Opens a new browser tab with Google Maps at clicked coordinates                                             | `window.open(https://www.google.com/maps?q=lat,lng)`           |
| 13  | Uploads at least one media file in opened draft workspace      | primary   | Draft marker is promoted to persistent media marker, remains visible on map                                 | Upload completion event + marker reconciliation                |
| 14  | Left-clicks map (or presses Escape) while draft has no uploads | primary   | Workspace draft session closes and empty draft marker is removed immediately                                | Draft cancel handler                                           |
| 15  | Clicks outside / presses Escape / taps backdrop (menu only)    | secondary | Closes context menu with no action                                                                          | dismiss handler                                                |
| 16  | Right-clicks on marker instead of empty map                    | primary   | Marker context menu wins; map context menu does not open                                                    | target disambiguation                                          |
| 17  | Right-clicks near viewport edges/corners                       | secondary | App menu is clamped to viewport bounds; browser menu placement is left to browser                           | clamp-only positioning                                         |
...
| `docs/specs/component/map/map-zone.md`                         | Cross-reference gesture precedence (menu vs radius drag)     |
| `docs/specs/component/map/radius-selection.md`                 | Clarify drag-threshold behavior when secondary press is used |
