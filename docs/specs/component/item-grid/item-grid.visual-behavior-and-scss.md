...
| Media selected emphasis    | `.media-item__slot`                       | `app-media-item:host`                    | `.media-item__open` and quiet-action buttons | `.media-item__slot--selected`             | layer/selected (2)    | secondary blue ring on slot only — not on square `:host` padding |
| Media upload overlay       | `.media-item__upload-overlay`             | `app-media-item:host`                    | none (passive overlay)                       | `.media-item__upload-overlay`             | layer/upload (3)      | upload overlay does not shift layout and stays below quiet actions |
| Media quiet actions reveal | `.media-item__quiet-actions`              | `app-media-item:host`                    | `.media-item-quiet-actions__button*`         | `.media-item__quiet-actions`              | layer/actions (4)     | reveal on slot hover/focus; select stays visible when selected |
| Media tile hover emphasis  | `.media-item__slot`                       | `app-media-item:host`                    | `.media-item__slot`                          | `.media-item__slot:hover`, `.media-item__slot--linked-hover` | layer/hover (1b) | primary gold; **box-shadow** only (no CSS `outline` on rounded slot) |
...
| Media selected emphasis    | `.media-item__slot`                       | `.media-item__slot--selected`              | `.media-item__slot--selected`            | ✅                                                                                 |
| Media quiet actions reveal | `.media-item__quiet-actions`              | `.media-item__slot:hover` / `--selected` | `.media-item__quiet-actions`              | ⚠️ exception — CSS cascade trigger only — no FSM state crosses component boundary. |
