...
| **Rounded control** | `var(--container-radius-control)` → **`var(--radius-md)`** (**Batch 44:** **`--container-radius-*`** on **`_typography-baseline.scss` `:root`**, **`--radius-*`** steps on **`styles.scss` `@theme inline`**) | Toolbar / operator control aligned with **buttons** | **Grouping**, **Filter**, **Sort**, **Projects** dropdown triggers (`hlmBtn` + toolbar menu-trigger classes) on map workspace and projects toolbars |
