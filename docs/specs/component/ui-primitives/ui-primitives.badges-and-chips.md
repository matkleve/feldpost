...
| **Rounded control** | `var(--container-radius-control)` → **`var(--radius-md)`** — spacing/radius scale: [`docs/design/tokens.md`](../../../design/tokens.md); toolbar menu shell: [`dropdown-system.md`](../filters/dropdown-system.md) | Toolbar / operator control aligned with **buttons** | **Grouping**, **Filter**, **Sort**, **Projects** dropdown triggers (`hlmBtn` + toolbar menu-trigger classes) on map workspace and projects toolbars |
