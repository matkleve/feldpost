...
| **Rounded control** | `var(--container-radius-control)` (`--radius-md` bridge) | Toolbar / operator control aligned with **buttons** | **Grouping**, **Filter**, **Sort**, **Projects** dropdown triggers (`hlmBtn` + toolbar menu-trigger classes) on map workspace and projects toolbars |
