...
The panel is a section shell of **intake**, a **three-option lane switch**, and **one visible lane**. Lane ids are `uploading`, `clarifications`, and `uploaded`. Labels are **Waiting**, **Clarification**, and **Uploaded**. Clarification holds unresolved jobs (duplicates, missing location, upload errors) and, in the grid shell, the resolver trays. Only the selected lane is mounted. The lane switch has **no extra card wrapper**. File rows use **surface** backgrounds with status tints; the list area uses a **transparent overflow wrapper** (max **5** visible rows, internal scroll).

**Wide layout (grid shell only).** When `app-upload-panel` is the grid-shell upload surface and its width is at least **42rem**, the shell is two columns: intake on the left, and on the right the same three options stacked **top to bottom** beside the selected lane. Below that width the three options stay in one horizontal row under the intake. The legacy upload shell stays single-column. **Reference tree:** [upload-panel.layout-and-states.md](upload-panel.layout-and-states.md). **Row menus and status text:** [upload-panel.lane-and-row-actions.md](upload-panel.lane-and-row-actions.md).
