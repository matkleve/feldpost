...
The panel is a section shell of **intake**, a **three-option lane switch**, and **one visible lane**. Lane ids stay `uploading`, `issues`, and `uploaded`. Labels are **Waiting**, **Clarification**, and **Uploaded**. Clarification holds unresolved jobs (duplicates, missing location, upload errors) and, in the grid shell, the resolver trays. Only the selected lane is mounted. The lane switch has **no extra card wrapper**. File rows use **surface** backgrounds with status tints; the list area uses a **transparent overflow wrapper** (max **5** visible rows, internal scroll).
...
| 4g   | Clicks **Import archive**                                                          | Same folder picker as 4, but batch is created with `importMode: 'archive'` (classify, no trays, unresolved → Issues) | [archive import mode](../../service/media-upload-service/upload-archive-import-mode.md) A1 — Phase 5.1 |
...
| 12   | Switches segmented control to Clarification (`issues`)                             | Lane list filters to unresolved jobs; grid shell also mounts the resolver tray in this lane                    | `selectedLane = 'issues'`                                  |
