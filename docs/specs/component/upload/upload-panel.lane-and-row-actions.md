...
flowchart LR
  ...
  B -->|clarifications duplicate_photo| E[Upload anyway<br/>Open existing media<br/>Dismiss]
  B -->|clarifications missing_gps| F[Add GPS<br/>Add/Change address<br/>Retry<br/>Dismiss]
  B -->|clarifications address_deferred| I[Add GPS<br/>Add/Change address<br/>Retry<br/>Dismiss]
  B -->|clarifications document_unresolved| H[Add GPS<br/>Add/Change address<br/>Assign project<br/>Dismiss]
  B -->|clarifications conflict_review| G[Resolve conflict<br/>Retry<br/>Dismiss]
```
...
| `clarifications: duplicate_photo`                  | dedupe review                                                | `Open existing media`, `Upload anyway`, `Dismiss`                                                                                                                                   | `Upload anyway` only for duplicate-photo rows                                                                                                                                  |
| `clarifications: missing_gps`                      | GPS/manual placement needed                                  | `Add GPS`, `Add/Change address`, `Retry`, `Dismiss`                                                                                                                                 | `Retry` re-queues only when retry preconditions are met                                                                                                                        |
| `clarifications: address_deferred`                 | user deferred address during tray resolution                 | `Add GPS`, `Add/Change address`, `Retry`, `Dismiss`                                                                                                                                 | Same recovery surface as `missing_gps`; written by tray defer path                                                                                                             |
| `clarifications: document_unresolved`              | document without GPS/address                                 | `Add GPS`, `Add/Change address`, `Assign project`, `Dismiss`                                                                                                                        | `Assign project` resolves geospatially missing documents as project-bound artifacts                                                                                            |
| `clarifications: address_ambiguous`                | ambiguous address prompt                                     | `Select suggested candidate`, `Enter location manually`, `Cancel`                                                                                                                   | Upload-internal prompt only; no map/workspace exposure                                                                                                                         |
| `clarifications: conflict_review` / `upload_error` | conflict or hard error                                       | `Retry`, `Dismiss`                                                                                                                                                                  | No force-upload actions in these issue kinds                                                                                                                                   |
...
| `awaiting_conflict_resolution`                | `Waiting for decision…`                      | `clarifications`    | Conflict review hold                       |
| `missing_data` + `missing_gps`                | `Choose location`                            | `clarifications`    | Requires GPS/address placement             |
| `missing_data` + `document_unresolved`        | `Choose location or project`                 | `clarifications`    | Resolvable by location or project binding  |
| `skipped` + `duplicate_photo`                 | `Already uploaded`                           | `clarifications`    | Duplicate review entry                     |
| `error`                                       | `Upload failed`                              | `clarifications`    | Retry path allowed when supported          |
...
| `clarifications: duplicate_photo`                    | `Open existing media`, `Upload anyway`                                                                                                                                                                | `Dismiss`                                                                                                                             | danger text + danger icon |
| `clarifications: missing_gps`                        | `Add GPS`, `Add/Change address`, `Retry`                                                                                                                                                              | `Dismiss`                                                                                                                             | danger text + danger icon |
| `clarifications: address_deferred`                   | `Add GPS`, `Add/Change address`, `Retry`                                                                                                                                                              | `Dismiss`                                                                                                                             | danger text + danger icon |
| `clarifications: document_unresolved`                | `Add GPS`, `Add/Change address`, `Assign project`                                                                                                                                                     | `Dismiss`                                                                                                                             | danger text + danger icon |
| `clarifications: address_ambiguous`                  | `Select suggested candidate`, `Enter location manually`                                                                                                                                               | `Cancel`                                                                                                                              | danger text + danger icon |
| `clarifications: conflict_review` or `upload_error`  | `Retry`                                                                                                                                                                                               | `Dismiss`                                                                                                                             | danger text + danger icon |
...
| Issue kind = `address_deferred`                  | Deferred tray address needs the same placement/retry recovery as missing GPS  |
...
| `assign_to_project`       | `Assign project`                               | `primary`     | `uploaded`, `clarifications` (`document_unresolved`) | for uploaded and unresolved-document resolution  | shared project selector available    | open shared project selector primitive   |
...
| `open_existing_media`     | `Open existing media`                          | `primary`     | `clarifications`                                     | `duplicate_photo`                                | `existingMediaId`                    | navigate to existing media               |
| `upload_anyway`           | `Upload anyway`                                | `primary`     | `clarifications`                                     | `duplicate_photo`                                | none                                 | force duplicate upload flow              |
| `add_gps_issue`           | `Add GPS`                                      | `edit`        | `clarifications`                                     | `missing_gps`, `address_deferred`, `document_unresolved`             | map-pick capability                  | placement workflow                       |
| `change_address_issue`    | `Add address` / `Change address`               | `edit`        | `clarifications`                                     | `missing_gps`, `address_deferred`, `document_unresolved`             | none                                 | address finder workflow                  |
| `candidate_select`        | `Select suggested candidate`                   | `primary`     | `clarifications`                                     | `address_ambiguous`                              | none                                 | apply chosen address candidate           |
| `manual_location_entry`   | `Enter location manually`                      | `secondary`   | `clarifications`                                     | `address_ambiguous`                              | address editor available             | open manual location entry               |
| `cancel_location_prompt`  | `Cancel`                                       | `destructive` | `clarifications`                                     | `address_ambiguous`                              | none                                 | close ambiguous-address prompt           |
| `retry`                   | `Retry`                                        | `primary`     | `clarifications`                                     | `missing_gps`, `address_deferred`, `conflict_review`, `upload_error` | retry preconditions met              | retry job                                |
| `dismiss`                 | `Dismiss`                                      | `destructive` | `clarifications`                                     | all issue kinds                                  | none                                 | dismiss row                              |
...
| Issue or failed (`clarifications` lane)  | `Dismiss`                                                                        | `dismissJob(jobId)`                                   |
...
- [ ] Embedded mode clears row selection when the user switches Queue / Uploaded / Clarifications lane tabs.
