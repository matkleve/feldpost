...
- `Queue` (Uploading): Compact control with icon + text (`cloud_upload` + `Queue`).
- `Uploaded`: Compact control with icon + text (`check_circle` + `Uploaded`).
- `Issues`: Compact control with icon + text (`warning_amber` + `Issues`), with attention styling when unresolved items exist.
...
| Missing user-facing explanation of why options appear/disappear in row dropdown                        | `docs/element-specs/component/upload-panel.md`, `apps/web/src/app/features/upload/upload-panel-item.component.ts`                                                                                                                      | Add explicit visibility/rationale matrix per lane/issue/state to make option gating transparent and testable.                                              | P1       |
