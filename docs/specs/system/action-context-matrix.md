...
The visual shell is still the shared dropdown/action-sheet shell ([`docs/specs/component/filters/dropdown-system.md`](../component/filters/dropdown-system.md); token names in [`docs/design/tokens.md`](../../design/tokens.md); layer ownership in [`docs/design/token-layers.md`](../../design/token-layers.md)), but section order is driven by this matrix rather than local menu code.
...
| `delete_locations`          | —          | —                | —         | —            | —                 | —                | ✅ (count guard)      | —                           | —               | Footer bulk only v1: unlink all location links per selected media; keeps files |
...
| `delete_locations`          | `ws_footer_multi(guarded)`                                                                                                                                  | Clears geo/address associations without deleting assets                 | Users may confuse with media delete; shared locations need scoped unlink | Footer toolbar only in v1; count guard + confirm; disable when no links   |
