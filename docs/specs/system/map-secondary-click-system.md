...
All secondary-click menus use the shared dropdown/action-sheet shell (`dd-*` rows) with the same elevated surface vocabulary as other floating menus: **`--color-bg-elevated`**, **`--color-border`**, **`box-shadow: var(--shadow-lg)`**, **`--radius-lg`**. **Authors:** tweakcn semantic names, shadow ladder, and stacking live in [`docs/design/tokens.md`](../../design/tokens.md); shell, `dd-*`, and plane rules are normative in [`docs/specs/component/filters/dropdown-system.md`](../component/filters/dropdown-system.md). Treat those two documents as the sole checklist for token names and menu chrome — not historical migration diffs. Desktop menus anchor to cursor/marker/radius hit zone. Mobile uses bottom action sheets for touch reliability.
