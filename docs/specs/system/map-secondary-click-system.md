...
All secondary-click menus use the shared dropdown/action-sheet shell (`dd-*` rows) with the same elevated surface vocabulary as other floating menus: **`--color-bg-elevated`**, **`--color-border`**, **`box-shadow: var(--shadow-lg)`**, **`--radius-lg`**. **Authors:** token bucket ownership in [`docs/design/token-layers.md`](../../design/token-layers.md); tweakcn semantic names, shadow ladder, and stacking in [`docs/design/tokens.md`](../../design/tokens.md); shell, `dd-*`, and plane rules in [`docs/specs/component/filters/dropdown-system.md`](../component/filters/dropdown-system.md). Treat those three documents as the sole checklist for global CSS custom properties and menu chrome — not historical migration diffs. Desktop menus anchor to cursor/marker/radius hit zone. Mobile uses bottom action sheets for touch reliability.
...
| `docs/specs/component/map/map-context-menu.md`                           | Map menu details                               |
| `docs/specs/ui/media-marker/media-marker-context-menu.md`                  | Marker menu details                            |
| `docs/specs/component/map/radius-selection.md`                           | Radius draw + radius menu details              |
