...
```text
ItemGridSystem
â”œâ”€â”€ ItemGridComponent (layout only, no domain knowledge)
â”‚   â”œâ”€â”€ mode classes: item-grid--grid-sm | --grid-md | --grid-lg | --row | --card
â”‚   â””â”€â”€ <ng-content select="app-*-item"> projected domain items
â”œâ”€â”€ ItemComponent (abstract base class, no layout geometry)
â”‚   â”œâ”€â”€ Shared state frame
â”‚   â”‚   â”œâ”€â”€ Loading layer (pulse placeholder)
â”‚   â”‚   â”œâ”€â”€ Error layer
â”‚   â”‚   â”œâ”€â”€ Empty layer
â”‚   â”‚   â””â”€â”€ No selected visual ownership (selection stays domain-owned)
â”‚   â””â”€â”€ Domain content outlet (overridden by subclasses)
â”œâ”€â”€ MediaItemComponent (extends ItemComponent)
â”‚   â”œâ”€â”€ MediaItemRenderSurfaceComponent (features/media; no shared wrapper)
â”‚   â”‚   â”œâ”€â”€ Layer: loading/content/error/no-media
â”‚   â”‚   â”œâ”€â”€ Row mode: dynamic height ratio from media metadata
â”‚   â”‚   â””â”€â”€ MediaItemUploadOverlayComponent (features/media)
â”‚   â””â”€â”€ MediaItemQuietActionsComponent (features/media; ws_grid_thumbnail actions)
â”œâ”€â”€ DocumentItemComponent (extends ItemComponent)
â”‚   â””â”€â”€ filetype-suitability contract: A4 behavior for eligible document-like previews
â”œâ”€â”€ ProjectItemComponent (extends ItemComponent)
â”‚   â”œâ”€â”€ Project metadata and status
â”‚   â””â”€â”€ Project actions bound to project context matrix contract
â””â”€â”€ JobItemComponent (extends ItemComponent, placeholder contract only)
    â””â”€â”€ Not implemented in this phase
```
...
| -------------------------- | ----------------------------------------------- | --------------------------------------------------- | --------------------------------------------------- | ---------------------------------------------------------- |
| Shared loading layer       | `.item-state-frame__state-layer--loading`       | `.item-state-frame__state-layer--loading`           | `.item-state-frame__state-layer--loading`           | âœ…                                                         |
| Media loading fallback     | `.media-item-render-surface__fallback--loading` | `.media-item-render-surface__fallback--loading`     | `.media-item-render-surface__fallback--loading`     | âœ…                                                         |
| Shared error layer         | `.item-state-frame__state-layer--error`         | `.item-state-frame__state-layer--error`             | `.item-state-frame__state-layer--error`             | âœ…                                                         |
| Shared empty layer         | `.item-state-frame__state-layer--empty`         | `.item-state-frame__state-layer--empty`             | `.item-state-frame__state-layer--empty`             | âœ…                                                         |
| Media selected emphasis    | `.media-item-render-surface__media-frame`       | `.media-item-render-surface__media-frame--selected` | `.media-item-render-surface__media-frame--selected` | âœ…                                                         |
| Media quiet actions reveal | `.media-item__quiet-actions`                    | `.media-item--selected` (parent state gate)         | `.media-item__quiet-actions`                        | âš ï¸ exception â€” reveal is intentionally parent-state driven |
