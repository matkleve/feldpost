...
- Runtime ownership target (service facade + adapters):
  - `apps/web/src/app/core/media-download.service.ts` (target facade)
  - `apps/web/src/app/core/photo-load.service.ts` (existing signed-url/cache backend)
  - `apps/web/src/app/core/media/media-orchestrator.service.ts` (existing tier policy backend)
  - `apps/web/src/app/core/zip-export.service.ts` (legacy client-side export backend, transition only)
...
| `apps/web/src/app/core/media/media-orchestrator.service.ts`                                             | Tier and fallback policy              | Tier Resolver adapter behind facade              |
| `apps/web/src/app/core/zip-export.service.ts`                                                           | Client ZIP orchestration              | Edge Export Orchestrator adapter + edge function |
| `apps/web/src/app/core/zip-export.helpers.ts`                                                           | ZIP helper ownership                  | Edge export helper ownership                     |
...
| `apps/web/src/app/core/media/media-orchestrator.service.ts`                                             | Existing tier and fallback policy backend to be adapted behind unified facade       |
| `apps/web/src/app/core/zip-export.service.ts`                                                           | Legacy export path kept only as migration fallback during edge cutover              |
...
| `apps/web/src/app/core/media-download.service.spec.ts`                                                  | Target unified contract tests (new)                                                 |
...
| ------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------- | --------------------------------------------------------------------------------------------------------- |
| `(new)` `apps/web/src/app/core/media-download.service.ts`                                               | `apps/web/src/app/core/media-download.service.ts` (facade bridge phase)                                                                                | Done             | New facade exists and bridges to legacy services; consumer imports remain unchanged in this phase         |
| `apps/web/src/app/core/photo-load.service.ts`                                                           | `apps/web/src/app/core/media-download/adapters/signed-url-cache.adapter.ts`                                                                            | In Progress      | `PhotoLoadService` is now deprecated and used as internal bridge by `MediaDownloadService.resolvePreview` |
| `apps/web/src/app/core/photo-load.model.ts`                                                             | `apps/web/src/app/core/media-download/adapters/signed-url-cache.adapter.types.ts`                                                                      | In Progress      | `MediaDelivery*` facade types introduced; alias extraction into adapter types still pending               |
| `apps/web/src/app/core/media/media-orchestrator.service.ts`                                             | `apps/web/src/app/core/media-download/adapters/tier-resolver.adapter.ts`                                                                               | In Progress      | `MediaOrchestratorService` is now deprecated and used for tier resolution via facade                      |
| `apps/web/src/app/core/media/media-renderer.types.ts`                                                   | `apps/web/src/app/core/media-download/adapters/tier-resolver.adapter.types.ts` (partial extraction)                                                    | Pending          | `MediaTier` ownership shifts; exports kept via barrel during transition                                   |
| `apps/web/src/app/core/zip-export.service.ts`                                                           | `apps/web/src/app/core/media-download/adapters/edge-export-orchestrator.adapter.ts` (client) + `supabase/functions/media-export-zip/index.ts` (server) | In Progress      | `ZipExportService` is now deprecated and wrapped by facade `exportSelection` bridge                       |
| `apps/web/src/app/core/zip-export.helpers.ts`                                                           | `apps/web/src/app/core/media-download/adapters/edge-export-orchestrator.helpers.ts` (or edge function helper module)                                   | Pending          | Helper ownership shifts from UI-local ZIP builder to edge export context                                  |
| `apps/web/src/app/core/upload/upload.service.ts` (`getSignedUrl`, `downloadFile`)                       | `apps/web/src/app/core/media-download.service.ts` facade delegation                                                                                    | Pending          | Upload service methods become pass-through and later removable from public upload API                     |
| `apps/web/src/app/core/workspace-view.service.ts`                                                       | Facade calls (`resolveBatchPreviews`)                                                                                                                  | Pending          | Direct `photoLoad.batchSign` removed from workspace service                                               |
| `apps/web/src/app/features/map/map-shell/map-shell.component.ts`                                        | Facade calls (`resolvePreview`, `invalidateStale`)                                                                                                     | Pending          | Direct `PhotoLoadService` import removed after compatibility phase                                        |
| `apps/web/src/app/features/media/media-item.component.ts`                                               | Facade calls (`resolvePreview`, `getItemState`)                                                                                                        | Pending          | Direct orchestrator/photo-load composition removed from component                                         |
| `apps/web/src/app/features/map/workspace-pane/media-detail-view.component.ts`                           | Facade calls for thumb/full + state                                                                                                                    | Pending          | Detail flow shifts from dual direct services to single facade contract                                    |
| `apps/web/src/app/features/map/workspace-pane/media-detail-data.facade.ts`                              | Facade calls for thumb/full signing                                                                                                                    | Pending          | Direct `PhotoLoadService` typed dependency removed                                                        |
| `apps/web/src/app/features/map/workspace-pane/thumbnail-grid.component.ts`                              | Facade edge export call                                                                                                                                | Pending          | `ZipExportService` dependency removed                                                                     |
| `apps/web/src/app/features/map/workspace-pane/workspace-pane-footer/workspace-pane-footer.component.ts` | Facade edge export call                                                                                                                                | Pending          | `ZipExportService` dependency removed                                                                     |
