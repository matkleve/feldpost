# Media Multi-Project Membership — Implementation Blueprint

> **Related specs:**
>
> - [element-specs/media-detail-view.md](../../element-specs/media-detail/media-detail-view.md)
> - [element-specs/media-detail-inline-editing.md](../../element-specs/media-detail/media-detail-inline-editing.md)
> - [element-specs/projects-dropdown.md](../../element-specs/projects-dropdown.md)
> - [element-specs/workspace/workspace-view-system.md](../../element-specs/workspace/workspace-view-system.md)

## Goal

Allow one media item (`images.id`) to belong to multiple projects at once, while keeping RLS organization boundaries intact and preserving existing workspace/project filters.
...
- `setMediaProjects(mediaId: string, projectIds: string[]): Promise<void>`
- `addMediaToProject(mediaId: string, projectId: string): Promise<void>`
- `removeMediaFromProject(mediaId: string, projectId: string): Promise<void>`
...
- Media detail quick info uses "Projects" summary chip.
...
- Unit: membership diff logic (`setMediaProjects`) including no-op, add-only, remove-only, mixed.
- Integration: project filter returns a media item when it has at least one selected project.
- Integration: a media item appears in multiple project-scoped views simultaneously.
