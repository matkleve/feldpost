...
**Note (2026-05-18):** `apps/web/src/styles/_legacy-design-tokens.scss` is **not in the tree**; `apps/web/src/styles.scss` has **no** `meta.load-css` for that path (remaining `meta.load-css` is for `styles/typography-baseline` only).
