...
- **Next:** [Phase 7 — token migration](./phase-7-token-migration.md) **in progress** — Batches **1–5b**, **6**; **14** (Feldpost v1 **`--color-*`** duplicate bridge removed from **`_legacy-design-tokens.scss`**); **15** (dead **`--fp-alias-*`** rows removed, **`figma-tokens.json`** regen); **16** (**`--fp-sys-color-*`** `:root` definitions removed — MD3 roles documented in **`tokens.md`** only); remaining **`--fp-sys-shape-*`** / spacing / elevation / typescale in legacy bridge; **§Risks:** Tailwind **`dark:`** ≠ system+OS dark until variant/ThemeService follow-up. Then 8–11.
- **Last updated:** 2026-05-17 (Phase 7 Batches **16**, **12–15** + 1–5b, 6; Phase 5 Group D / Phase 6 Gate B; [tokens.md §3.1a — Phase 7 handoff](../design/tokens.md#phase-7-handoff--deferred-md3-rows-tweakcn)).
...
| 7 — Token system unification | [phase-7-token-migration.md](./phase-7-token-migration.md) | **In progress (2026-05-17)** — Batches 1–5b + **6**; **12**–**16** (Batch **14**: v1 **`--color-*`** bridge rows removed; Batch **15**: **`--fp-alias-*`** dead rows + `figma-tokens.json`; **Batch 16**: **`--fp-sys-color-*`** definitions removed from legacy bridge); **13** (**§4 `hlm-toggle-group` / CVA**). Open: remaining **`--fp-sys-shape-*`** / non-color **`--fp-sys-*`**, **§Risks** (`dark:` vs semantic system dark). |
