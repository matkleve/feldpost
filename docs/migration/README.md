...
- **Next:** [Phase 7 — token migration](./phase-7-token-migration.md) **in progress** — **Batch 27** removed dead **layout / overlay-width / elevation-base** rows and **zero-consumer** shadow, border, outline, and **`--interactive-shadow-*`** aliases from **`_legacy-design-tokens.scss`** (grep table in phase-7 doc). Bridge still carries **motion**, **active layout** (`--container-*`, `--overlay-rail-*`, …), **semantic surface** aliases (`--action-*`, `--menu-*`, …), and the **single** modular **`--font-size-*`** ladder (no **`LEGACY MAPPING`** font block). Remaining Phase 7: shrink **still-used** bridge where call sites migrate to tweakcn/Tailwind; **§Risks** follow-ups. Then Phases 8–11.
- **Last updated:** 2026-05-17 — Phase **6** closed; Phase **7** **Batch 27** (legacy bridge shrink + design doc sync); **Batch 26** (post–font **`LEGACY MAPPING`** rg audit); **Batch 25** (font alias block removed); Phase 5 Group D; [tokens.md §3.1a — Phase 7 handoff](../design/tokens.md#phase-7-handoff--deferred-md3-rows-tweakcn).
