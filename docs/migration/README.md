...
- **Next:** [Phase 7 — token migration](./phase-7-token-migration.md) **in progress** — **Batch 28** removed **dead metric** **`--border-sm`…`--border-xl`** from **`_legacy-design-tokens.scss`** (zero **`var(--border-(sm|md|lg|xl))`** consumers — see phase-7 §Batch 28). **Batch 27** removed dead **layout / overlay-width / elevation-base** rows and **zero-consumer** shadow, outline, and **`--interactive-shadow-*`** aliases (plus semantic border/outline names **`--border-default`** etc.). Bridge still carries **motion**, **active layout** (`--container-*`, `--overlay-rail-*`, …), **`--border-hover` / `--border-selected`**, **semantic surface** aliases (`--action-*`, `--menu-*`, …), and the **single** modular **`--font-size-*`** ladder (no **`LEGACY MAPPING`** font block). Remaining Phase 7: shrink **still-used** bridge where call sites migrate to tweakcn/Tailwind; **§Risks** follow-ups. Then Phases 8–11.
- **Last updated:** 2026-05-17 — Phase **6** closed; Phase **7** **Batch 28** (metric border scale removal + **`tokens.md`** §3.5); **Batch 27** (legacy bridge shrink + design doc sync); **Batch 26** (post–font **`LEGACY MAPPING`** rg audit); **Batch 25** (font alias block removed); Phase 5 Group D; **Phase 8 Path A** (map-shell Leaflet → **`_map-shell-leaflet-global.scss`** / **`app-map-shell`** scope — [phase-8 §7](./phase-8-global-scss-elimination.md#7-inventory-remaining-styles-tree)); [tokens.md §3.1a — Phase 7 handoff](../design/tokens.md#phase-7-handoff--deferred-md3-rows-tweakcn).
...
| 7 — Token system unification | [phase-7-token-migration.md](./phase-7-token-migration.md) | **In progress (2026-05-17)** — **`LEGACY MAPPING`** font-size aliases **removed (Batch 25)**; **Batch 28** removed unused metric **`--border-sm`…`--border-xl`** (§Batch 28 `rg` proof). **Batch 27** dead bridge row removal (layout/overlay/shadow/semantic border-outline — see §Batch 27 `rg` table). **Batch 26** rg/doc pass (no alias stragglers). Modular **`var(--font-size-*)`** + motion/layout/semantic rows remain in **`_legacy-design-tokens.scss`**. Prior batches **1–5b**, **6**, **12**–**24** in phase-7 doc. Open: **13** (**§4 `hlm-toggle-group` / CVA**). **§Risks** (`dark:` vs semantic system dark). |
| 8 — Global SCSS elimination | [phase-8-global-scss-elimination.md](./phase-8-global-scss-elimination.md) | **In progress (2026-05-17)** — `styles/primitives/` already gone; **Path A shipped:** pierced Leaflet map-shell rules hoisted to **`styles/_map-shell-leaflet-global.scss`** with **`app-map-shell { … }`** scope (**`styles.scss`** `@use`); former partials **`_map-shell-photo-markers.scss`**, **`_map-shell-photo-marker-states.scss`**, **`_map-shell-radius-location-markers.scss`** **deleted** (rules global). **`anyComponentStyle`** clean on latest **`ng build`**; global **`styles`** chunk carries the trade-off (see phase-8 §7). Remains: **`hlm-toggle-group.scss`** teardown, minimal **`styles.scss`** `@use` set. **While bridge remains:** [tokens.md §3.1a — Phase 7 handoff](../design/tokens.md#phase-7-handoff--deferred-md3-rows-tweakcn). |
