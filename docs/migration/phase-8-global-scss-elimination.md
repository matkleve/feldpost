...
**Progress (2026-05-16, slice — §7 `load-css` + reset review):** Documented non-`@use` global emits: ~~**`@include meta.load-css('styles/legacy-design-tokens')`**~~ **removed 2026-05-18** — stub **`_legacy-design-tokens.scss`** emits no **`:root`** properties (Batch 50). **`@include meta.load-css('styles/typography-baseline')`** → **`apps/web/src/styles/_typography-baseline.scss`** (after **`@layer base`** / Preflight). **`reset.scss`:** **`html, body`** **`font-family`** now uses **`var(--font-sans)`** with system fallbacks (removes hardcoded **Inter** drift from tweakcn **DM Sans**).
...
- [x] **`styles.scss`** without **`@include meta.load-css('styles/legacy-design-tokens')`** — **shipped 2026-05-18** (stub file comment-only / zero emit; **`typography-baseline`** still requires **`meta.load-css`** after **`@import "tailwindcss"`** + **`@layer base`** — mid-file **`@use`** remains invalid Sass for that slot; see **`phase-7-token-migration.md`** § **Legacy bridge `meta.load-css` — historical constraints** and **`apps/web/src/styles.scss`** header).
...
- **Phase 7 legacy bridge file** — **`_legacy-design-tokens.scss`** kept as **disk stub** for migration cross-refs; **delete** when **`tokens.md` / `token-layers.md`** no longer anchor inventory there. **`typography-baseline`** **`load-css`** ordering constraints unchanged (**`phase-7-token-migration.md`** § **Legacy bridge `meta.load-css` — historical constraints**; **`apps/web/src/styles.scss`** header).
- **Phase 10** — unchecked DoD sign-off line above.

### Next levers (suggested sequencing)

- ~~Drop **`@include meta.load-css('styles/legacy-design-tokens')`**~~ **Done (2026-05-18)** — stub emits nothing.
- Optional: delete **`_legacy-design-tokens.scss`** after doc inventory moves off that path.
