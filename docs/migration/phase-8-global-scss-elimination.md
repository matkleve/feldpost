...
- **Wave P3 hygiene slice (2026-05-18, `::ng-deep` elimination + `rg` proof):** **`rg '::ng-deep' apps/web/src/app --glob '*.scss'`** → **0** matches (post-slice). **Prior state:** **4** matches in **3** files — eliminated without new pierces (Phase 10: **no new** **`::ng-deep`**).
  ...
  | **`styles`** initial-chunk row (this gate run) | **`cd apps/web && npx ng build 2>&1 \| rg 'styles-[A-Z0-9]+\.css \| styles'`** | **`styles-U46CN7B5.css | styles | 81.44 kB | 11.83 kB`** (unchanged vs prior doc baseline; hash tied to global CSS artifact). |
  **Dead selectors:** none removed this slice (no orphan selector **`rg`** hits). **Ongoing:** §7 runbook + occasional row scrape; dead-selector **`rg`** on **`map-shell*.scss`** when touching map SCSS. **`apps/web/src/styles/_map-shell-leaflet-global.scss`** line **1** — comment text **`was :host ::ng-deep`** only (not a pierce).
