...
**Retained (still consumed):** **`--shadow-focus`**, modular **`--font-size-2xs`…`--font-size-2xl`** + **`--motion-duration-fast`** + **`--motion-ease-out`** on **`_typography-baseline.scss`** `:root` after **Batch 42** (was legacy bridge); **`--radius-sm|md|lg|full`**. **`--interactive-transition-standard`** removed **Batch 41** (see §Batch 41). **Batch 39** removed **`--border-hover`** (inlined **`media-item.component.scss`**). **Batch 35:** settings overlay min widths (**`25rem`** / **`32rem`**) and photo-marker hit/dot/selected literals live at call sites (not on `:root`).
...
**Goal:** Remove three **`_legacy-design-tokens.scss`** `:root` z-index rows; preserve stacking intent with literal **`z-index`** at callsites and numeric **`tailwind.config.js`** **`theme.extend.zIndex`** entries (**`200`**, **`300`**, **`500`**). **`calc(var(--z-dropdown) + 2)`** → **`302`**; **`calc(var(--z-modal) + 1)`** → **`501`**.
