...
### Unblocker — replace **`meta.load-css('styles/legacy-design-tokens')`** with top **`@use`** (2026-05-17)

**Verdict:** not safe in **one** wave without a larger pipeline or file split.

| Step | Action |
|------|--------|
| 1 | Read **`apps/web/src/styles.scss`** header comment (global emit order §1–§5): bridge is intentionally **after** tweakcn **`:root`** / **`html[data-theme="…"]`** / system-dark / sandstone so downstream **`@theme inline`** still sees semantic vars first. |
| 2 | **Do not** add **`@use './styles/legacy-design-tokens'`** next to the existing top **`@use`** cluster: Dart Sass forbids **`@use`** after **`@import "tailwindcss"`** and other non-`@use` rules, and hoisting would emit bridge CSS **before** tweakcn + Tailwind — **cascade / selector order** vs **`[data-theme='dark']`**, **`html[data-theme="dark"]`**, and **`@media (prefers-color-scheme: dark)`** blocks in both files is **not** byte-identical without a full CSS-order audit + visual/token regression. |
| 3 | **Escape hatch (optional, not default):** same line, **`@import './styles/legacy-design-tokens'`** likely preserves emit order vs **`load-css`**, but keeps **deprecated `@import`** (Dart Sass 3.0 path) — trade deprecation debt vs removing **`meta`**. |
| **Prerequisite** | **Shrink** **`_legacy-design-tokens.scss`** until the remainder can be **inlined** into **`styles.scss`** beside tweakcn / **`@theme`**, or introduce an **`angular.json`** multi-entry **`styles`** pipeline with a documented order contract. |

**Risks:** duplicate **`@layer`** / duplicate **`:root`** keys if merge is sloppy; dark/system selector mismatch between **`[data-theme='dark']`** (bridge) and **`html[data-theme="dark"]`** (tweakcn) if order changes.
...
**Retained (still consumed):** **`--shadow-focus`**, **`--border-hover`**, remaining **`--motion-duration-fast|slow`**, **`--motion-ease-out`**, transition aliases, modular **`--font-size-*`**, **`--radius-sm|md|lg|full`**. **Batch 35:** settings overlay min widths (**`25rem`** / **`32rem`**) and photo-marker hit/dot/selected literals live at call sites (not on `:root`).

**Deferred:** Motion ladder removal (**many** `var(--motion-*)` call sites); **`--line-height-*`** / **`--font-weight-*`** (wide `var()` use); **`--content-clamp-*`**, remaining sidebar geometry. **Batch 36 doc drift** (**`token-layers.md`** Layer B, **`motion.md`**, specs, **`feldpost-component` SKILL**): closed same-day — see **`docs/design/tokens.md`** §3.6.
...
**Retained:** **`--border-hover`** (**`--border-selected`** removed **Batch 36** — use **`0.125rem solid var(--primary)`** at the sole callsite), **`--field-border: var(--border-strong)`** (tweakcn **`--border-strong`** in **`styles.scss`**).
...
**Retained (still consumed):** remaining **`--line-height-*`**, **`--radius-sm|md|lg|full`**, **`--transition-fade-*`**, **`--container-radius-control|panel`**, **`--section-bg`**, **`--section-text`**, **`--state-warning-bg`**, **`--state-danger-bg`**.
...
| --- | --- | --- |
| `--layout-sidebar-width-collapsed` | **2** | **`nav.component.scss`**, **`settings-overlay.component.scss`** → **`var(--spacing-7)`** |
...
| `--font-size-3xl` | **1** | **`settings-overlay.component.scss`** → **`calc(var(--font-size-2xl) * 1.13)`**; bridge **`--font-size-4xl`** rewired to **`calc(var(--font-size-2xl) * 1.13 * 1.13)`** |
...
**Documentation (Batch 37 deferred — closed 2026-05-17):** **`dropdown-system.md`**, **`media-marker-context-menu.md`**, **`map-secondary-click-system.md`**, **`map-context-menu.md`** — prescribe **`box-shadow: var(--shadow-lg)`** (removed **`--elevation-dropdown`**); physical stack is canonical in **`docs/design/tokens.md`** §3.5.
...
**Deferred (doc prose — file cap):** **`docs/specs/ui/media-marker/media-marker.md`**, **`docs/design/design-system/layout-width-breakpoint-scale.md`**, **`docs/design/design-system/page-layout-framework.md`**, **`docs/design/token-layers.md`** — still cite **`--photo-marker-body-size`** / **`--content-clamp-*`** / **`--menu-item-text-active`**; align in a follow-up batch.
...
| Canonical modular scale (expected many hits) | `rg 'var\(--font-size-(2xs|xs|sm|md|lg|xl|2xl|4xl)\)' apps/web` | **3xl** not emitted on `:root` after **Batch 33** — use **`calc(var(--font-size-2xl) * 1.13)`** where needed |

**What still lives in `_legacy-design-tokens.scss` after Batch 25:** tweakcn-facing **semantic** rows (shadows, borders, **`--motion-*`** primitives, z-index ladder, **`--spacing-*`**, **`--radius-*`**, layout / overlay / marker geometry tokens, **`--action-*` / `--menu-*` / `--field-*` / `--section-*` / `--state-*`**, dark mixin overrides) — **not** a second deprecated font alias map. **Batch 37** dropped **`--elevation-*`**, **`--transition-fade-*`**, and **`--animation-skeleton-pulse`** — use **`var(--shadow-sm|md|lg)`** and motion primitives / literal skeleton animation at callsites (see §Batch 37).
