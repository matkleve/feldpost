...
## Remaining work (post-closure)

**None blocking Phase 7 closure (2026-05-19).**

| Item | Owner | Notes |
|------|-------|-------|
| Spec / **`tokens.md` parity on edit** | [Phase 11](./phase-11-spec-sync.md) hygiene | Normative tables must not imply **`_legacy-design-tokens.scss`** on disk |
| **`theme-toggle.md` vs settings overlay** | Phase 11 | Four-mode toggle in **`SettingsOverlayComponent`** (`light` \| `dark` \| `system` \| `sandstone`); see § **Risks / QA** doc-drift note |
| **`tailwind.config.js` full v4 token migration** | Phase 5 / 8 | **Out of Phase 7 DoD** — runtime follows **`@custom-variant`** in **`styles.scss`** |
| Deferred motion / literal choreography | Optional | Optional consolidation from batch notes; not blocked on removed bridge |
...
rg 'var\(--fp-sys-|var\(--fp-' apps/web/src/app --glob "*.scss" -l
rg 'var\(--color-|var\(--fp-' apps/web/src --glob "*.scss" -l
```
...
rg 'var\(--fp-sys-|var\(--fp-' apps/web/src/app --glob "*.scss" -c
```
...
**Still out of scope for this batch (Batch 1 note):** historical **`_legacy-design-tokens.scss`** emissions surface (file **removed** from **`apps/web`**); **`--fp-sys-*` / retired reference palette variables** in non–`src/app` SCSS.
...
**Batch 5b resolution:** retired reference palette variables `:root` definitions **removed** (canonical hex → `docs/design/tokens.md` §3.1a). **Batch 16 resolution:** MD3 **`--fp-sys-color-*`** `:root` definitions **removed** (same doc tables). App SCSS gate **`var(--color-|var(--fp-`**. (**System OS dark + tweakcn** resolved in **Batch 4** — see below.)
...
# Retired reference-palette definition gate (2026-05-16): see docs/archive/design-retired-reference-tokens.md
...
|---------|------------------------------------------|-----------------|-------------------------------------|
| `var(--fp-` | **0** | **0** | — |
| retired reference palette | **0** | **0** | — (**Batch 5b**, 2026-05-16: tonal + typeface reference definitions removed from `_legacy-design-tokens.scss`; canonical hex → `docs/design/tokens.md` §3.1a / §3.1e) |
...
**Interpretation:** No **application** SCSS/TS/HTML under `apps/web` references **`var(--fp-*)`**. Retired MD3 reference palette custom properties are no longer defined on `:root` (see **§ Batch 5b** and [`docs/archive/design-retired-reference-tokens.md`](../archive/design-retired-reference-tokens.md)). **`--fp-sys-color-*`** are no longer defined on `:root` (see **§ Batch 16**). Remaining non-color **`--fp-sys-*`** definitions were removed in **§ Batch 17**.
...
- **Reference tonal ladders (Batch 5b):** Hex authority in **`docs/design/tokens.md` §3.1a / §3.1e** — use tweakcn / semantic tokens in implementation. Retired variable naming: archive link above.

### Batch 5b — reference palette bridge removal + doc canonical hex (2026-05-16)

**Audit record (retired token literals, grep gates):** [`docs/archive/design-retired-reference-tokens.md`](../archive/design-retired-reference-tokens.md).

**Summary:** Removed all reference-palette `:root` definitions from **`_legacy-design-tokens.scss`** (five tonal ladders + typeface lines). **`docs/design/tokens.md`** §3.1a / §3.1e hold canonical stop hex. **`chip.md`** documents **`color-mix`** + stop **95/90** pointers. **Verify:** `cd apps/web && npx ng build` → exit **0**.

### Batch 6 — Doc sync: discovery + panel-trigger alias prose (2026-05-17)

**Slice:** Markdown only — completes part of the Batch 5b follow-up list (doc alignment to **`tokens.md` §3.1a** stop tables).

| File | Change |
|------|--------|
| `docs/migration/phase-0-discovery.md` | Global style bullet: retired reference palette variables no longer on `:root`; bridge + **`docs/design/tokens.md`** §3.1a as canonical reference stops. |
...
**Goal:** After Batch 17, **`--fp-*`** custom properties are **not emitted** from this bridge; only comments reference historical **`--fp-sys-*` / retired reference palette variables**. Remaining shrink target is the **`/* LEGACY MAPPING (DEPRECATED) */`** `:root` block (typography aliases → canonical **`--font-size-2xs`** … **`--font-size-4xl`**).
...
**Proof (zero residual):** *(second `rg` targeted the legacy bridge file — **absent** from current **`apps/web`**; replay via **`git show <pre-Batch-50>:apps/web/src/styles/_legacy-design-tokens.scss`** if needed.)*
...
**Deferred:** Motion ladder removal (**many** `var(--motion-*)` call sites); **`--font-weight-*`** bridge consumption; **`--line-height-comfortable`** (sole remaining line-height row on the legacy bridge) and any future consolidation of **`_typography-baseline.scss`** **`--line-height-tight|solid|reading`** call sites; remaining sidebar geometry. (**Batch 38** closed **`--content-clamp-*`**, **`--container-inset*`**, marker body, and menu-active-text **doc** drift — see §Batch 38 doc wave.) **Batch 36 doc drift** (**`token-layers.md`** Layer B, **`motion.md`**, specs, **`docs/skills/feldpost-component/SKILL.md` (removed 2026-09-10; content promoted to `.cursor/rules/` + component-structure skill)**): closed same-day — see **`docs/design/tokens.md`** §3.6.
...
**Documentation (Batch 36 deferred, file cap):** closed same-day — **`docs/design/token-layers.md`** Layer B, **`docs/design/motion.md`**, **`docs/skills/feldpost-component/SKILL.md` (removed 2026-09-10; content promoted to `.cursor/rules/` + component-structure skill)**, and choreography tables in **`project-item`**, **`item-grid.state-and-fsm`**, **`item-state-frame`**, **`media-item-quiet-actions`**, **`panel-trigger`** specs now use **`200ms`** / **`200ms cubic-bezier(0.4, 0, 0.2, 1)`** / **`var(--motion-ease-out)`** per **`docs/design/tokens.md`** §3.6 (no prescriptions of removed **`var(--motion-duration-base)`** / **`--motion-ease-standard`**).
...
**Context:** Post–**Batch 49**, **`rg '^\s*--[a-zA-Z0-9-]+:'`** on **`_legacy-design-tokens.scss`** was already **0** — the **`:root { … }`** block held **only** **`//`** history comments (no emitted custom properties). **Batch 50** (**2026-05-17**) removed that scaffold; **2026-05-18** dropped **`@include meta.load-css('styles/legacy-design-tokens')`** from **`styles.scss`** and **deleted** **`apps/web/src/styles/_legacy-design-tokens.scss`** from the repo (**no** on-disk stub — chronicle stays in this doc and prior batch sections; **`@use`** after Tailwind remains forbidden — see § **Legacy bridge `meta.load-css` — historical constraints**).
...
Static verification; browser sign-off recorded separately (**§ Browser sign-off (2026-05-19)**). **`cd apps/web && npx ng build`** → exit **0** (2026-05-18).
...
| **(3)** | Theme **System**, OS **dark** | **Pass (browser, 2026-05-19)** — desk gap **resolved** | **System** removes `data-theme` → `:root:not([data-theme])` applies dark **semantic** vars; **`dark:`** utilities **stay off** (variant is explicit-dark only) — **by design**. **Desk mismatch (2026-05-18):** `toast-variants.ts` used `dark:bg-green-900/20` on success — **fixed:** success/warning/error rows use semantic **`border-success/20 bg-success/10 text-success`** (and **`warning`** / **`destructive`** equivalents); **`rg 'dark:' apps/web/src`** → comment-only. **Browser:** settings overlay theme toggle, map chrome, toast surfaces verified under System+OS dark |
...
- **`dark:` footprint:** **`rg 'dark:' apps/web/src`** (excluding `node_modules`, build artifacts) → **zero** product callsites after **`toast-variants.ts`** semantic rewrite (**success** / **warning** / **error** use **`--success`**, **`--warning`**, **`--destructive`** token utilities). Comment-only hits in `styles.scss` / `tailwind.config.js`.
...
- **Doc drift (Phase 11):** `docs/specs/ui/settings-overlay/theme-toggle.md` still describes **`ThemeService`** / `feldpost-theme` / three-state cycle — implementation is **settings overlay** four-mode toggle (`light` \| `dark` \| `system` \| `sandstone`); **deferred** to [Phase 11](./phase-11-spec-sync.md) — does not block Phase 7 closure.
...
**§ Risks / QA closure (desk):** Superseded by **Browser sign-off (2026-05-19)** below — steps **(1)–(4)** exercised in browser.

### Browser sign-off (2026-05-19)

**Human verification (product UI):** User confirmed all theme paths work in browser — **Dark**, **Light**, **System + OS dark**, **Sandstone** (settings overlay theme control; map/nav chrome; toast semantic surfaces). Aligns with desk audit steps **(1)–(4)**.

| Step | Scenario | Browser | Notes |
|------|----------|---------|-------|
| **(1)** | Theme **Dark** | **Pass** | `data-theme="dark"` — semantic vars + surfaces consistent |
| **(2)** | Theme **Light**, OS **dark** | **Pass** | Forced light palette; no OS-dark leak on primary chrome |
| **(3)** | Theme **System**, OS **dark** | **Pass** | Semantic dark via `:root:not([data-theme])`; toasts use semantic success/warning (post-**`toast-variants.ts`** fix) |
| **(4)** | **Sandstone**, OS **dark** | **Pass** | Warm sandstone surfaces; no erroneous global dark palette on `:root:not([data-theme])` |

**Code evidence (post-sign-off):** `apps/web/src/app/shared/ui/toast/toast-variants.ts` — **`success`**: `border-success/20 bg-success/10 text-success`; **`warning`**: `border-warning/20 bg-warning/10 text-warning` (no **`dark:`** utilities).

**§ Risks / QA:** **Closed** for Phase 7 theme matrix. **Phase 10** full screen matrix remains separate ([phase-10-visual-qa.md](./phase-10-visual-qa.md)).
...
| Legacy **`--color-*` / `--fp-sys-*` / retired reference palette variables** handoffs in app SCSS | `rg 'var\(--color-|var\(--fp-' apps/web/src/app --glob '*.scss'` | **0** lines |
...
| retired reference palette variables | **Never in components** — if encountered, replace with semantic or remove dead code | |
...
3. **Map tokens** (marker colors, cluster halo, selection rings)  
   - **Done (2026-05-19 closure re-run):** `rg 'var\(--color-|var\(--fp-' apps/web/src/app/features/map --glob '*.scss'` → **0**; map chrome uses tweakcn / **`--map-*`** semantics in **`styles.scss`** and map-shell SCSS (no legacy **`--color-*`** hand-offs).
...
2. Replace each `var(--color-*)` / `var(--fp-sys-*)` / `var(retired reference palette CSS variables)` with the mapped semantic `var(--…)` from tweakcn.
...
| No legacy color / fp-sys in app SCSS | `rg 'var\(--color-|var\(--fp-' apps/web/src/app --glob "*.scss"` → **zero** (explicit waiver list empty unless archived code deleted) |
...
**Notes (closure re-run 2026-05-19):** All gates re-executed; evidence in § **Closure verification** below.

---

## Closure verification (2026-05-19)

Phase 7 **close** wave — acceptance gates re-run from repo root:

| Gate | Command | Result (2026-05-19) |
|------|---------|---------------------|
| No legacy **`var(--fp-*)`** in app SCSS | `rg 'var\(--fp-' apps/web/src/app --glob '*.scss'` | **0** matches (exit **1**) |
| No legacy color / fp- prefixes in app SCSS | `rg 'var\(--color-\|var\(--fp-' apps/web/src/app --glob '*.scss'` | **0** matches (exit **1**) |
| Legacy bridge path in shipped web tree | `rg 'legacy-design-tokens\|_legacy-design-tokens' apps/web` | **0** matches (exit **1**) |
| Build | `cd apps/web && npx ng build` | exit **0** (CommonJS dependency warnings only) |

Supersedes § **Wave P2 closeout (2026-05-18)** for closure sign-off; Wave P2 table remains historical audit.

**Recheck (2026-05-19, independent):** Acceptance gates re-run from repo root (not doc-only). All four commands match the table above: `rg 'var\(--fp-' apps/web/src/app --glob '*.scss'` → **0** (exit **1**); `rg 'var\(--color-|var\(--fp-' apps/web/src/app --glob '*.scss'` → **0** (exit **1**); `rg 'legacy-design-tokens|_legacy-design-tokens' apps/web` → **0** (exit **1**); `styles.scss` has **no** `@use './styles/tokens'` and **no** `load-css('styles/legacy-design-tokens')` (only `meta.load-css('styles/typography-baseline')`); `cd apps/web && npx ng build` → exit **0** (CommonJS warnings only). Spot-checks: product `dark:` only in `styles.scss` comment (`@custom-variant dark`); `toast-variants.ts` uses semantic tokens (`bg-background`, `border-success/20`, `text-destructive`, etc.); `apps/web/src/styles/**/*.scss` → **7** files, no `_legacy-design-tokens.scss`. Cross-docs: § **Remaining work** has no blocking rows; [README](./README.md) Phase **7** = **100**; [open-questions.md](./open-questions.md) Phase 6 / CDK items N/A or resolved; [phase-8 Preconditions](./phase-8-global-scss-elimination.md#preconditions) cites this closure.

---

## Definition of done

- [x] Acceptance table green (§ **Closure verification**, **2026-05-19**).
- [x] `docs/migration/open-questions.md` — Phase 6 **`tokens.scss`** gate and CDK overlay import notes updated (**2026-05-19**).
- [x] Phase 8 pre-flight — [phase-8 Preconditions](./phase-8-global-scss-elimination.md#preconditions) references this closure re-run (**2026-05-19**).
