...
**Retained (still consumed):** **`--shadow-focus`**, **`--motion-duration-fast`**, **`--motion-ease-out`**, transition aliases, modular **`--font-size-2xs`…`--font-size-2xl`** ( **`--font-size-4xl`** removed **Batch 40** — inline **`calc(var(--font-size-2xl) * 1.13 * 1.13)`** ), **`--radius-sm|md|lg|full`**. **Batch 39** removed **`--border-hover`** (inlined **`media-item.component.scss`**). **Batch 35:** settings overlay min widths (**`25rem`** / **`32rem`**) and photo-marker hit/dot/selected literals live at call sites (not on `:root`).
