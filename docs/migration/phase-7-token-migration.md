...
**Retained (still consumed):** **`--shadow-focus`**, **`--motion-duration-fast`**, **`--motion-ease-out`**, modular **`--font-size-2xs`…`--font-size-2xl`** ( **`--font-size-4xl`** removed **Batch 40** — inline **`calc(var(--font-size-2xl) * 1.13 * 1.13)`** ), **`--radius-sm|md|lg|full`**. **`--interactive-transition-standard`** removed **Batch 41** (see §Batch 41). **Batch 39** removed **`--border-hover`** (inlined **`media-item.component.scss`**). **Batch 35:** settings overlay min widths (**`25rem`** / **`32rem`**) and photo-marker hit/dot/selected literals live at call sites (not on `:root`).

**Deferred:** Motion ladder removal (**many** `var(--motion-*)` call sites); **`--line-height-*`** / **`--font-weight-*`** (wide `var()` use); remaining sidebar geometry. (**Batch 38** closed **`--content-clamp-*`**, **`--container-inset*`**, marker body, and menu-active-text **doc** drift — see §Batch 38 doc wave.) **Batch 36 doc drift** (**`token-layers.md`** Layer B, **`motion.md`**, specs, **`feldpost-component` SKILL**): closed same-day — see **`docs/design/tokens.md`** §3.6.
...
**Retained through Batch 28:** semantic hover border **`--border-hover`** (removed **Batch 39** — inlined **`media-item.component.scss`**). **`--border-selected`** removed **Batch 36** — use **`0.125rem solid var(--primary)`** at the sole callsite. **`--field-border: var(--border-strong)`** (tweakcn **`--border-strong`** in **`styles.scss`**).
...
**Goal:** Remove **≥10** legacy bridge rows using **duplicate-name** proof (tweakcn **`styles.scss`** already defines **`--shadow-md|lg|xl`** before `meta.load-css`; legacy only overwrote) or **single-/dual-consumer inlines** (exclude **`_legacy-design-tokens.scss`** from consumer counts; exclude **`apps/web/src/styles.scss`** self-bridge rows from counts).
...
**Theme mirrors:** **`@mixin dark-theme-overrides`** — dropped **`--shadow-md|lg|xl`**, **`--border-hover`**, **`--photo-marker-drop-shadow`** (kept warm **`--shadow-sm`**, composite **`--shadow-focus`**). **`[data-theme='sandstone']`** — dropped **`--action-border-active`**, **`--menu-surface-bg`**, **`--menu-item-bg-active`**, **`--section-bg`**, **`--state-warning-bg`**, **`--state-danger-bg`** (**`--section-text`** retained).
...
**Deferred (Batch 41 / file cap):** **`--line-height-comfortable`** (still on legacy **`:root`**); optional **shared SCSS mixin** for the repeated **`120ms ease-out`** transition list now duplicated at four callsites; further **`--z-upload-button|dropdown|modal`** literal inlines when a follow-up batch has headroom.
...
| Canonical modular scale (expected many hits) | `rg 'var\(--font-size-(2xs|xs|sm|md|lg|xl|2xl)\)' apps/web` | **3xl** not emitted on `:root` after **Batch 33** — use **`calc(var(--font-size-2xl) * 1.13)`** where needed; **4xl** not on `:root` after **Batch 40** — use **`calc(var(--font-size-2xl) * 1.13 * 1.13)`** at callsites |

**What still lives in `_legacy-design-tokens.scss` after Batch 25:** tweakcn-facing **semantic** rows (warm **`--shadow-sm`**, **`--shadow-focus`**, **`--motion-duration-fast`** / **`--motion-ease-out`**, z-index ladder minus **`--z-panel` (Batch 41)**, **`--spacing-*`** minus **`--spacing-7` (Batch 41)**, **`--radius-*`**, **`--container-radius-control|panel`**, **`--action-bg-hover`**, **`--action-text-*`**, **`--menu-*`**, **`--field-*`**, **`--line-height-comfortable`**, dark mixin overrides) — **not** a second deprecated font alias map. **Batch 41** moved **`--line-height-tight|solid|reading`** to **`_typography-baseline.scss`** `:root` and inlined seven other line-height / motion / z / spacing bridge rows (see §Batch 41). **Batch 39** dropped duplicate **`--shadow-md|lg|xl`** plus **`--border-hover`**, **`--photo-marker-drop-shadow`**, **`--action-border-active`**, **`--menu-surface-bg`**, **`--menu-item-bg-active`**, **`--section-bg`**, **`--state-warning-bg`**, **`--state-danger-bg`** (see §Batch 39). **Batch 40** dropped ten alias/trivial/leaf rows (see §Batch 40). **Batch 37** dropped **semantic `--elevation-subtle|overlay|dropdown`**, **`--transition-fade-*`**, and **`--animation-skeleton-pulse`** — use **`var(--shadow-sm|md|lg|xl)`** and motion primitives / literal skeleton animation at callsites (see §Batch 37). (**Batch 35** dropped **`--elevation-modal`**.)
