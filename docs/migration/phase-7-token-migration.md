...
| **Removal gate** | **Met:** stub is **`//`** comments only → **`load-css`** deleted; **`meta.load-css('styles/typography-baseline')`** unchanged (headings after Preflight). |
...
|---------|--------------------------------------|
| `var(--color-` | **0** files in `apps/web/src/app` (`*.scss`) — **current:** bridge stub emits **no** **`--*`** (historical: **`var(--color-*)`** lived only in **`_legacy-design-tokens.scss`** until **Batch 14**) |
...
**Still out of scope for this batch (Batch 1 note):** **`_legacy-design-tokens.scss`** as an **emissions** surface (superseded — file is **non-loaded**, comment-only stub); **`--fp-sys-*` / `--fp-ref-*`** in non–`src/app` SCSS.
...
**Goal:** **2026-05-17** — remove the final **`:root {}`** scaffold so **`_legacy-design-tokens.scss`** is a **no-CSS-output** comment stub. **2026-05-18** — remove **`@include meta.load-css('styles/legacy-design-tokens')`** from **`styles.scss`** (no-op once the partial emits nothing; **`@use`** after Tailwind remains forbidden — see **`styles.scss`** “Legacy bridge” comment).

**Post–Batch 50 shipped:** **`styles.scss`** does not **`load-css`** the stub; disk file **kept** for migration **`@see`** pointers until references move.

**Pre-edit proof (repo root):**

```bash
rg '^\s*--[a-zA-Z0-9-]+:' apps/web/src/styles/_legacy-design-tokens.scss
# → 0 matches (no --* property definitions on bridge before Batch 50)
rg -n '^:root\s*\{' apps/web/src/styles/_legacy-design-tokens.scss
# → 1 match (comment-only :root block — removed in Batch 50)
```
...
rg '^\s*--[a-zA-Z0-9-]+:' apps/web/src/styles/_legacy-design-tokens.scss
# → 0 matches
rg ':root' apps/web/src/styles/_legacy-design-tokens.scss
# → 0 matches
wc -l apps/web/src/styles/_legacy-design-tokens.scss
# → 6 lines (stub `//` header; no rules)
```
...
**Files touched (5):** **`apps/web/src/styles/_legacy-design-tokens.scss`**, **`docs/design/tokens.md`**, **`docs/design/token-layers.md`**, **`docs/migration/phase-7-token-migration.md`**, **`docs/migration/README.md`**.

**Deferred:** optional **delete** of **`apps/web/src/styles/_legacy-design-tokens.scss`** after all **`@see`** / migration references point elsewhere (no build impact today — file is not imported).

**Verify:** `npm run design-system:check` → exit **0**; `cd apps/web && npx ng build` → exit **0**.

### Batch 26 — post–**`LEGACY MAPPING`** font block audit (2026-05-17)

**Goal:** Confirm no **`apps/web`** regressions to removed deprecated **`var(--font-size-<alias>)`** names (Batch 19 inventory / §Batch 25 proof); align docs with **post–Batch-50 emission-empty stub** bridge reality.
