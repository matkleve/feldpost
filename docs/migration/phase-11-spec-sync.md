...
- [ ] **Legacy bridge inventory parity** — when Phase 7 shrinks **`_legacy-design-tokens.scss`**, diff specs and **`tokens.md`** / **`token-layers.md`** bridge tables together (no normative **`var(--…)`** for removed **`--*`** names).
