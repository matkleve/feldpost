...
- [ ] **Legacy bridge inventory parity** — **`apps/web/src/styles/_legacy-design-tokens.scss` is absent** (Phase 7 Batch **50** + file removal); keep specs and **`tokens.md`** / **`token-layers.md`** free of normative **`var(--…)`** for removed **`--*`** names — scrub doc **`@see`** paths that still point at the deleted filename.
